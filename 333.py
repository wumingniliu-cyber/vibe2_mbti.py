...
def render_html(html_str):
    """安全渲染 HTML，防止 Streamlit 将缩进误认为 Markdown 代码块"""
...
.block-container { padding-top: 3rem !important; padding-bottom: 4rem !important; max-width: 1300px !important; }
...
div[data-testid="stSelectbox"] > div > div { background-color: rgba(4, 9, 20, 0.9) !important; border: 1px solid rgba(168, 85, 247, 0.5) !important; color: #a855f7 !important; font-weight: bold;}
...
/* Tabs 全息导航坞 */
[data-testid="stTabs"] button { color: #64748b !important; font-family: 'Noto Sans SC', sans-serif !important; font-weight: 900 !important; font-size: 15px !important; padding-bottom: 15px !important; transition: all 0.3s ease; }
...
def trigger_supernova():
    html_str = ""
    for _ in range(35): 
        tx = random.uniform(250, 900) * math.cos(random.uniform(0, 2 * math.pi))
        ty = random.uniform(250, 900) * math.sin(random.uniform(0, 2 * math.pi))
        text = random.choice(["DATA", "NODE", "HASH", "ASSET", "SYNC", "SDE"])
        html_str += f'<div class="firework-center" style="--tx:{tx}px; --ty:{ty}px; --s:{random.uniform(1.0, 3.0)}; --rot:{random.randint(-360, 360)}deg; animation-delay:{random.uniform(0, 0.15)}s; font-size:{random.randint(14, 24)}px;">{text}</div>'
...
# 🚨 18 维满血防爆字典：彻底补齐所有字典键值对，告别 KeyError 与数据缺失
mbti_details = {
