...
# ==============================================================================
# 🚨 强制左对齐并去除多余换行，避免 Markdown 将 HTML 错误解析为代码块
global_css = """
<style>
...
[data-testid="stCodeBlock"] > div, [data-testid="stCodeBlock"] pre, [data-testid="stCodeBlock"] code { background-color: #050505 !important; border-color: #333 !important; }
[data-testid="stCodeBlock"] pre, [data-testid="stCodeBlock"] code { font-family: 'Fira Code', monospace !important; font-size: 12px !important; color: #10b981 !important; line-height: 1.5 !important;}
[data-testid="stCodeBlock"] > div { border-left: 3px solid #10b981 !important; border-radius: 6px !important; }
...
if not st.session_state.started:
    ...
    with center_container():
        # 🚨 取消所有前置空格，杜绝 HTML 暴露
        html_title = """<div style="text-align: center; margin-bottom: 20px;">
...
<div class="hero-subtitle">PRO_MAX_BUILD</div>
        ...
        html_term = """<div class="terminal-container">
...
<span class="cursor-blink"></span>
    ...
    with center_container():
        ...
        html_q = f"""<div style="background: rgba(10, 15, 25, 0.9); border: 1px solid rgba(0, 243, 255, 0.4); border-radius: 8px; padding: 30px; box-shadow: 0 10px 30px rgba(0,0,0,0.8), inset 0 0 20px rgba(0, 243, 255, 0.05); margin-top: 20px; margin-bottom: 30px; border-left: 5px solid #00f3ff;">
...
<div style="font-size: 18px; color: #ffffff !important; line-height: 1.8; font-weight: 700;">{q_data['q']}</div>
    ...
    # 🔐 资产不可篡改锁：🚨 完全修复 JSON 报 NameError Bug！将所有计算打入字典固化。
    if "asset_minted" not in st.session_state:
        ...
        # 将所有核心衍生哈希值存入 session 防止刷新时突变报错
    ...
    syn_tech, syn_biz, syn_comp = asset["syn_tech"], asset["syn_biz"], asset["syn_comp"]
    ...
    html_banner = f"""<div style="background: linear-gradient(135deg, rgba(16,185,129,0.1), rgba(10,10,30,0.9)); border: 1px solid #10b981; border-radius: 8px; padding: 15px 25px; margin-bottom: 25px; font-family: 'Orbitron', monospace; box-shadow: 0 0 20px rgba(16,185,129,0.2);">
...
<div style="font-size:12px; color:#a855f7; margin-bottom:8px; font-family:Orbitron; letter-spacing:2px; font-weight:bold;">[ SKILL TREE ]</div>
{skills_html_web}
</div>
...
<div style="color:#e2e8f0 !important; font-size:14px; line-height:1.8; margin-bottom:20px; font-weight:400; text-align:justify;">{data['desc']}</div>
        ...
        # 🚨 全新架构：双向对抗能量条！清晰展示对立面属性，并彻底干掉暴露代码 Bug！
        html_bars = f"""<div style="background:rgba(0,0,0,0.5); border:1px solid rgba(255,255,255,0.1); border-radius:8px; padding:25px 20px; margin-top:-10px;">
...
<div class="stat-bar-container" style="margin-bottom:0;"><div class="stat-bar-fill" style="width:{val_J}%; background:#10b981; box-shadow:0 0 10px #10b981;"></div></div>
    ...
    with col_r:
        ...
        with t_evo:
            ...
            html_evo = f"""<div style="border-left:3px solid #ffd700; padding-left:15px; background:linear-gradient(90deg, rgba(255,215,0,0.15), transparent); padding-top:10px; padding-bottom:10px; border-radius: 0 6px 6px 0; margin-left: 0px; box-shadow: 0 0 20px rgba(255,215,0,0.1); margin-bottom:25px;">
<div style="color:#ffd700; font-family:Orbitron; font-size:11px; margin-bottom:2px; letter-spacing:1px;">ULTIMATE DOMINANCE</div><div style="color:#ffd700; font-weight:900; font-size:18px;">{data['evolution']}</div>
</div>"""
            st.markdown(html_evo.replace('\n', ''), unsafe_allow_html=True)
            
            with st.expander("⚠️ 绝密防线：SDE 史诗级黑天鹅宕机推演 (点击解密)"):
                html_swan = f"""<div style="padding: 5px 10px; font-size: 14px; color: #cbd5e1; line-height: 1.7;">
...
contract SDE_Talent_Registry_V1 is ERC721 {{
    ...
    function executeMint() public {{
        ...
        with t_syn:
            ...
            html_syn = f"""<div style="background: rgba(59,130,246,0.1); border: 1px solid rgba(59,130,246,0.5); padding: 30px; border-radius: 12px; margin-top:20px; text-align:center; box-shadow: 0 0 30px rgba(59,130,246,0.15);">
...
# 🛑 [ CORE 07 ] 赛博呼吸专属版权区
# =========================================================================
html_footer = f"""<div style="text-align:center; margin-top:80px; margin-bottom:40px; position:relative; z-index:10;">
