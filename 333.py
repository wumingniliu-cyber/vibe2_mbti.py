...
# 🛠️ 终极防爆工具：强行剥离所有前置空格，杜绝 Streamlit 错误将 HTML 识别为代码块！
...
.block-container { padding-top: 2.5rem !important; padding-bottom: 4rem !important; max-width: 1450px !important; }
...
.mbti-code { font-family: 'Orbitron', sans-serif !important; font-size: clamp(60px, 8vw, 80px); font-weight: 900; color: #ffd700 !important; line-height: 1.1; letter-spacing: 6px; text-shadow: 0 0 35px rgba(255,215,0,0.6); margin: 5px 0;}
...
.panel-box { background: rgba(10, 17, 32, 0.7); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; padding: 25px; box-shadow: inset 0 0 20px rgba(0,0,0,0.4); margin-bottom: 20px;}
...
/* Tabs 导航坞 */
[data-testid="stTabs"] button { color: #64748b !important; font-family: 'Noto Sans SC', sans-serif !important; font-weight: 900 !important; font-size: clamp(14px, 2vw, 16px) !important; padding-bottom: 15px !important; transition: all 0.3s ease; }
...
/* 🚨 彻底锁死代码块深色极客风，杜绝浅色模式灰底污染！ */
div[data-testid="stCodeBlock"], div[data-testid="stCodeBlock"] > div, div[data-testid="stCodeBlock"] pre, div[data-testid="stCodeBlock"] code { background-color: #050505 !important; border-color: #333 !important; color: #10b981 !important; }
div[data-testid="stCodeBlock"] pre, div[data-testid="stCodeBlock"] code { font-family: 'Fira Code', monospace !important; font-size: 12px !important; line-height: 1.5 !important;}
...
# --- 🚀 全新重构！纯 GPU 硬件加速的量子代码扫描 (EMP) 解锁特效 (替代原本卡顿的 666) ---
def trigger_cyber_activation():
    html_str = """
    <style>
        .cyber-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: #030712; z-index: 999999; display: flex; justify-content: center; align-items: center; animation: cyber-fadeout 2.2s cubic-bezier(0.8, 0, 0.2, 1) forwards; pointer-events: none; }
        .cyber-line { position: absolute; width: 100vw; height: 2px; background: #00f3ff; box-shadow: 0 0 20px #00f3ff; animation: scan-split 1.5s cubic-bezier(0.8, 0, 0.2, 1) forwards; }
        .cyber-text { font-family: 'Orbitron', sans-serif; font-size: clamp(20px, 4vw, 46px); font-weight: 900; color: #fff; letter-spacing: 8px; text-shadow: 0 0 20px #00f3ff; overflow: hidden; white-space: nowrap; border-right: 4px solid #00f3ff; }
        .cyber-text::before { content: "SDE_NODE_AUTHORIZED"; animation: typing 0.8s steps(30, end) forwards, blink-caret 0.4s step-end infinite, text-glow 1.5s 0.5s forwards; }
        @keyframes typing { from { width: 0; } to { width: 100%; max-width: 650px; } }
        @keyframes blink-caret { 50% { border-color: transparent; } }
        @keyframes scan-split { 0% { transform: scaleY(1); opacity: 1; } 50% { transform: scaleY(120); opacity: 0.05; } 100% { opacity: 0; } }
        @keyframes text-glow { 0% { opacity: 1; } 100% { opacity: 0; transform: scale(1.1); filter: blur(8px); } }
        @keyframes cyber-fadeout { 0%, 75% { opacity: 1; } 100% { opacity: 0; visibility: hidden; } }
    </style>
    <div class="cyber-overlay"><div class="cyber-line"></div><div class="cyber-text"></div></div>
    """
...
with st.sidebar:
    render_html("<div style='text-align:center; font-family:Orbitron; font-size:24px; font-weight:900; color:#00f3ff; margin-bottom:20px; text-shadow: 0 0 15px rgba(0,243,255,0.6);'>SDE CORE V1.0</div>")
    render_html("<div style='font-size:11px; color:#94a3b8; font-family:Orbitron; border-bottom:1px solid #334155; padding-bottom:5px; margin-bottom:15px; font-weight:bold;'>/// NETWORK STATUS</div>")
    ...
    render_html("<div style='font-size:11px; color:#94a3b8; font-family:Orbitron; border-bottom:1px solid #334155; padding-bottom:5px; margin-top:30px; margin-bottom:15px; font-weight:bold;'>/// LIVE TX LOGS</div>")
    ticker_logs = "".join([f"<div style='font-family:\"Fira Code\", monospace; font-size:10px; margin-bottom:12px; border-left:2px solid #10b981; padding-left:8px;'><span style='color:#10b981;'>[MINT]</span> <span style='color:#e2e8f0;'>{random.choice(['Gov_Public_Data', 'Med_Research_Set', 'Fin_Risk_API', 'Logistics_GPS', 'Retail_Behavior'])}</span><br><span style='color:#ffd700;'>GAS: {random.randint(12000,45000)} Gwei</span></div>" for _ in range(6)])
    render_html(ticker_logs)

# ==============================================================================
# 🧠 [ CORE 04 ] 题库全量防残缺归位：40 道极客业务题库 (绝无删减)
# ==============================================================================
questions = [
    # E / I 维度 (10题)
...
# 🚨 18 维满血防爆字典 (向你 txt 原版数据中注入了高阶维度，绝对杜绝任何 KeyError)
mbti_details = {
    "INTJ": {
        ...
        "evolution_path": ["L1 架构规划官", "L2 核心规则主理人"],
    ...
    "INTP": {
        ...
        "evolution_path": ["L1 风控分析师", "L2 模型主理人"],
    ...
    "ISTJ": {
        ...
        "evolution_path": ["L1 审查风控官", "L2 规则执行官"],
    ...
    "ESTJ": {
        ...
        "evolution_path": ["L1 战区指挥官", "L2 跨域推进者"],
    ...
    "INFJ": {
        ...
        "evolution_path": ["L1 行业分析师", "L2 战略规划官"],
    ...
    "INFP": {
        ...
        "evolution_path": ["L1 体验叙事者", "L2 品牌调性官"],
    ...
    "ENTJ": {
        ...
        "evolution_path": ["L1 开拓先锋", "L2 战区统帅"],
    ...
    "ENTP": {
        ...
        "evolution_path": ["L1 沙盒破坏者", "L2 跨界重组官"],
    ...
    "ENFJ": {
        ...
        "evolution_path": ["L1 渠道统筹", "L2 联盟主理人"],
    ...
    "ENFP": {
        ...
        "evolution_path": ["L1 宣发先锋官", "L2 流量矩阵中枢"],
    ...
    "ISFJ": {
        ...
        "evolution_path": ["L1 运营专员", "L2 平台质检官"],
    ...
    "ESFJ": {
        ...
        "evolution_path": ["L1 商务专员", "L2 政企主理"],
    ...
    "ISTP": {
        ...
        "evolution_path": ["L1 底层架构师", "L2 灾备指挥官"],
    ...
    "ISFP": {
        ...
        "evolution_path": ["L1 UI视觉专员", "L2 交互总监"],
    ...
    "ESTP": {
        ...
        "evolution_path": ["L1 突击交易员", "L2 战地狼王"],
    ...
    "ESFP": {
        ...
        "evolution_path": ["L1 品牌代言", "L2 首席发言人"],
...
def calculate_synergy(mbti1, mbti2):
    ...
    if diff == 0: return 92, "【绝对镜像】决策回路高度一致，沟通0延迟，但需警惕认知盲区严重重叠导致集体翻车。"
    elif diff == 1: return 98, "【黄金握手】核心逻辑一致且具备极佳微调互补性，堪称 SDE 最强业务推土机小队！"
    elif diff == 2: return 85, "【灰度平衡】思维角度存在差异，能通过激烈碰撞打磨出更抗风险、更完美的业务闭环。"
    elif diff == 3: return 65, "【高频摩擦】存在极大的底层通信壁垒，协同作业时必须强制引入第三方确立明确的中间协议。"
    else: return 99, "【极致反转】代码完全相反！在日常沟通中极度痛苦，但若各司其职背靠背，能实现无死角的全域包抄！"
...
if not st.session_state.started:
    ...
    with center_container():
        ...
        <h1 class="hero-title" data-text="算力实战仪表盘" style="font-size:32px !important;">算力实战仪表盘</h1>
        <div class="hero-subtitle">▶ SDE MATRIX V1.0_PRO_MAX</div>
        </div>
        """)
        ...
        <div class="term-line"><span style="color:#94a3b8;">[SYSTEM]</span> Loading Talent Capability Algorithm V1.0... <span style="color:#00f3ff;">[OK]</span></div>
        ...
        您的物理能力将被<b>「全息要素化」</b>，系统将为您生成独一无二的<b>高阶数字凭证与上链算力估值</b>。</span>
        ...
        with st.form(key="login_form", border=False):
            render_html("<div style='color:#00f3ff; font-family:\"Orbitron\", sans-serif; font-size:12px; font-weight:bold; margin-bottom:8px; text-align:center;'>▼ MOUNT NODE ALIAS (输入职场称呼 / 授权代号) ▼</div>")
            st.text_input("", key="login_input", placeholder="例如：Compliance_Wu", label_visibility="collapsed")
            render_html("<br>")
            st.form_submit_button("▶ 开始算力测试并提取模型特征", on_click=start_assessment_callback, type="primary", use_container_width=True)

elif st.session_state.calculating:
    with center_container():
        render_html("<h2 class='hero-title' data-text='[ MINTING TO SDE CHAIN ]' style='font-size:28px !important; margin-top:50px; text-align:center; display:block;'>[ MINTING TO SDE CHAIN ]</h2>")
        mint_box = st.empty()
        hash_logs = ""
        for _ in range(12):
            fake_hash = hashlib.sha256(str(random.random()).encode()).hexdigest().upper()
            hash_logs = f"<span style='color:#94a3b8;'>[TX_HASH]</span> <span style='color:#ffd700;'>0x{fake_hash[:24]}...</span> <span style='color:#10b981;'>[CONFIRMED]</span><br>" + hash_logs
            mint_box.markdown(f"<div class='cli-box' style='height:250px; overflow:hidden; border-color:#ffd700;'>{hash_logs}</div>", unsafe_allow_html=True)
            time.sleep(0.12)
    ...
    with center_container():
        ...
        render_html(f"""
        <div style="background: rgba(2, 6, 23, 0.85); border: 1px solid rgba(0, 243, 255, 0.4); border-radius: 6px; padding: 25px; box-shadow: 0 10px 30px rgba(0,0,0,0.5), inset 0 0 15px rgba(0, 243, 255, 0.05); margin-top: 15px; margin-bottom: 25px; border-left: 5px solid #00f3ff;">
        ...
        <div style="font-size: 16px; color: #ffffff !important; line-height: 1.7; font-weight: 500;">{q_data['q']}</div>
        </div>
        """)
    ...
    # 🚨 40题极限算法归一化 (每维10题，极值[-20, 20]，映射至 [0, 100])
    ...
    # 🔐 资产防篡改锁：防止下拉框重渲染导致的数据跳变与 NameError
    ...
    # 1. 核心首屏区：左侧身份大卡，右侧互动沙盘 (手机端会变为上下排布)
    ...
    with col_top_r:
        # 👑 UX：将社交高频交互直接前置，无需下拉到底！
        render_html("<h4 style='color:#3b82f6 !important; border-left:4px solid #3b82f6; padding-left:10px; font-weight:900;'>🤝 跨域算力协同沙盘</h4>")
        render_html("<div style='font-size:13px; color:#94a3b8; margin-bottom:15px;'>输入团队成员或领导的核心架构代码，系统将立刻测算你们的协同匹配度：</div>")
        ...
        render_html("<h5 style='color:#10b981; margin-top:20px; margin-bottom:10px;'>💡 黄金搭档建议：</h5>")
        render_html(f"""
        ...
        您目前的最佳生态拍档为：<b style='color:#10b981; font-size:16px;'>{data['partner']}</b><br><br>{data['partner_advice']}
        </div>
        """)
    ...
    with col_mid_l:
        render_html("<h4 style='color:#00f3ff !important; border-left:4px solid #00f3ff; padding-left:10px; font-weight:900;'>🕸️ 核心算力雷达矩阵</h4>")
        fig_radar = go.Figure()
        ...
        # 🚨 全新重构双向对抗能量条 (绝对免疫 Markdown 代码块污染！)
        render_html(f"""
    ...
    with col_mid_r:
        render_html("<h4 style='color:#f43f5e !important; border-left:4px solid #f43f5e; padding-left:10px; font-weight:900;'>🎛️ 业务风控熔断仪</h4>")
        render_html(f"<div style='text-align:center; font-size:16px; font-weight:bold; color:{r_color}; font-family:Noto Sans SC; margin-top: 5px;'>{r_tag}</div>")
        ...
        render_html("<h4 style='color:#a855f7 !important; border-left:4px solid #a855f7; padding-left:10px; font-weight:900; margin-top:20px; margin-bottom:15px;'>🗄️ 算力深潜控制台 (DEEP DIVE)</h4>")
        render_html("<div style='text-align:right; font-size:12px; color:#94a3b8; margin-bottom:10px; opacity:0.8;'>👉 手机端可左右滑动切换面板</div>")
        ...
        with t_evo:
            ...
            <div style="color:#ffd700; font-family:Orbitron; font-size:11px; margin-bottom:2px; letter-spacing:1px;">PHASE 3 (ULTIMATE DOMINANCE)</div><div style="color:#ffd700; font-weight:900; font-size:18px;">{data['evolution']}</div>
            </div>
            """)
        ...
        with t_mkt:
            ...
            render_html(f"<div style='font-size:14px; color:#e2e8f0; margin-bottom:10px;'>基于随机游走推演的交易风格：<br><span style='color:#ffd700; font-weight:bold; font-size:15px;'>【 {data['market_style']} 】</span></div>")
            fig_roi = go.Figure()
        ...
        with t_3d:
            ...
            render_html("<div style='font-size:13px; color:#94a3b8; margin-bottom:10px;'>将多维指标降维映射至三维空间 (支持鼠标 360° 拖拽)。背景点阵为抽样全网节点。</div>")
        ...
        with t_sol:
            ...
            render_html("<div style='font-size:12px; color:#94a3b8; margin-bottom:10px;'>系统已自动为您生成专属的以太坊 ERC721 确权智能合约源码。</div>")
    ...
    # =========================================================================
    # 💠 4. 【沉底提取中心】防白屏海报与 JSON 协议下发 (全宽展示)
    # =========================================================================
    render_html("<h4 style='color:#00f3ff !important; border-left:5px solid #00f3ff; padding-left:12px; font-weight:900; margin-top:40px; margin-bottom:20px;'>💠 数据要素大屏提取终端</h4>")
    ...
    with t_img:
        ...
        # 🚨 终极安全锁：代码在 components.html 内，不受 Python Markdown 影响。
...
<div style="font-size:13px; font-weight:bold; color:#fff; text-shadow:0 0 10px rgba(255,0,60,0.6);">{data['evolution']}</div>
</div>
...
🔗 [Token ID: #{token_id} | Hash: 0x{hash_code[:8]}]"""
    ...
    with t_json:
        ...
        export_data = {
            ...
            "ultimate_evolution": data['evolution'],
