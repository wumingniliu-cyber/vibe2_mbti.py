...
# ==============================================================================
VERSION = "7.0_ULTIMATE_TCG_EDITION"
COPYRIGHT = "无名逆流"
SYS_NAME = "SDE 核心算力引擎 | 卡牌觉醒 V7.0"

st.set_page_config(page_title=SYS_NAME, page_icon="🃏", layout="wide", initial_sidebar_state="collapsed")

# ==============================================================================
# 🎨 [ SDE TCG 02 ] 史诗级卡游 UI 渲染底座 (全顶格压平防 Markdown 污染)
# ==============================================================================
GLOBAL_CSS = """
<style>
...
[data-testid="stTabs"] button { color: #64748b !important; font-family: 'Noto Sans SC', sans-serif !important; font-weight: 900 !important; font-size: clamp(13px, 3.5vw, 16px) !important; padding-bottom: 12px !important; transition: all 0.3s ease; }
...
# 📊 彭博社级深度盘口 HTML (杜绝跨行引发的 NameError)
def get_market_depth_html(hash_int):
    ...
    b_html = "".join([f"<div style='flex:1; background:linear-gradient(to top, rgba(16,185,129,0.1), rgba(16,185,129,0.8)); height:{b*100}%; border-top:1px solid #10b981; margin-right:1px; border-radius:2px 2px 0 0;'></div>" for b in bids])
    a_html = "".join([f"<div style='flex:1; background:linear-gradient(to top, rgba(244,63,94,0.1), rgba(244,63,94,0.8)); height:{a*100}%; border-top:1px solid #f43f5e; margin-left:1px; border-radius:2px 2px 0 0;'></div>" for a in asks])
    spread = round(rng.uniform(0.01, 0.05), 4)
    return f"""<div style="display:flex; height: 80px; align-items:flex-end; width:100%; margin-bottom:15px; border-bottom: 1px solid #334155; padding-bottom:5px;"><div style="flex:1; display:flex; align-items:flex-end; height:100%; padding-right:5px;">{b_html}</div><div style="width:2px; height:100%; background:#94a3b8; margin:0 2px;"></div><div style="flex:1; display:flex; align-items:flex-end; height:100%; padding-left:5px;">{a_html}</div></div><div style="display:flex; justify-content:space-between; font-size:10px; color:#94a3b8; font-family:'Orbitron', monospace; margin-bottom:20px;"><span>BIDS VOL</span><span style="color:#00f3ff; font-weight:bold;">SPREAD: {spread}</span><span>ASKS VOL</span></div>"""

# 📊 随机生成竞价订单墙 HTML
def generate_bids(hash_int):
    rng = np.random.RandomState(hash_int)
    companies = ["0xAI_Core", "ByteMatrix", "G-Cloud", "Web3_Unicorn", "FinTech_Giant", "Cyber_Sec"]
    bids = []
    for _ in range(4):
        comp = rng.choice(companies)
        size = rng.randint(10, 99) * 1000
        premium = round(rng.uniform(5.5, 35.5), 1)
        bids.append(f"""<div class='order-row'><span style='width:40%; text-align:left;'>{comp}</span><span style='width:30%; text-align:center;'>{size:,}</span><span style='width:30%; text-align:right;'>+{premium}%</span></div>""")
    return "".join(bids)

# ==============================================================================
# 🧠 [ SDE V7 ] 题库全量无损归位 (40 题)
# ==============================================================================
questions = [
...
# 🚨 18 维卡游级属性字典 (全部硬编码，绝不丢失任何描述)
mbti_details = {
    "INTJ": {"role": "首席数据架构师", "tier": "UR", "tier_color": "#ff003c", "rarity": "Top 1.2%", "base_hash": 9850, "desc": "数据要素底座的“造物主”，致力于为错综复杂的数字经济构建严密的底层制度与逻辑规则。", "tags": ["顶层设计", "逻辑闭环", "制度自信"], "partner": "ENTJ", "partner_advice": "将战略落地全权交由 ENTJ 推进，把 INTP 作为高维模型的逻辑验算机，您只需稳控全局架构不跑偏。", "tasks": ["主导 SDE 核心确权底层逻辑架构设计", "重构下一代高并发撮合交易引擎逻辑"], "black_swan": "过度追求底层架构完美闭环。面临突发政策转向时，系统极易因过于重型而无法敏捷掉头。", "patch": "在构建宏大的交易规则体系时，请适当为前台业务预留“沙盒容错”空间；捕获一线的非结构化反馈。", "skills": ["【被动】全知视界", "【法术】底层规则解构", "【领域】绝对秩序统御"], "artifact": "【架构师的真理圆规】", "base_roi": 1.45, "volatility": 0.20, "market_style": "宏观架构对冲与长期趋势跟踪策略", "evolution_path": ["L1 架构规划官", "L2 核心规则主理人"], "ultimate_evolution": "【绝对算力主宰】掌控数据产品的终极业务定价权"},
    "INTP": {"role": "量化风控专家", "tier": "SSR", "tier_color": "#ffd700", "rarity": "Top 3.1%", "base_hash": 9620, "desc": "穿透数据迷雾，寻找复杂业务表象下的底层逻辑漏洞与确权定价模型的最优解。", "tags": ["深度解构", "模型驱动", "极客思维"], "partner": "INTJ", "partner_advice": "依托 INTJ 将您的理论模型锚定在现实业务框架内，并借助 ENTP 的发散思维寻找商业变现出口。", "tasks": ["研发基于特征因子的数据资产动态定价算法", "建立实时数据异常交易嗅探与阻断模型"], "black_swan": "陷入“分析瘫痪”。在需要极速拍板的确权灰度地带，过度追求模型最优解往往导致商机流失。", "patch": "尝试将您极其高维的理论模型降维封装，形成非技术人员也能看懂的《操作指南》，让算法模型转化为生产力。", "skills": ["【被动】多维特征抽取", "【法术】零日漏洞嗅探", "【秘技】量子坍缩推演"], "artifact": "【破壁者的沙盒之刃】", "base_roi": 1.60, "volatility": 0.45, "market_style": "高频统计套利与多因子量化模型", "evolution_path": ["L1 风控分析师", "L2 模型主理人"], "ultimate_evolution": "【全知算法先知】构建百分百无损的跨网底层风控引擎"},
    "ISTJ": {"role": "合规审查主理官", "tier": "SR", "tier_color": "#a855f7", "rarity": "Top 11.6%", "base_hash": 8850, "desc": "SDE 底层防线的守夜人，您的评估本身就是安全、严谨与业务零失误的代名词。", "tags": ["绝对合规", "程序正义", "风险兜底"], "partner": "ESTJ", "partner_advice": "配合 ESTJ 建立坚不可摧的业务推进流水线，由 ISFJ 负责兜底后勤细节，形成最硬核的交付闭环。", "tasks": ["主导头部数商“数据资源入表”全链路审计", "设计业务合同与智能合约的合规映射SOP"], "black_swan": "过度依赖既有 SOP。面临无先例创新业务时，容易因“无库可查”产生本能的排斥与误杀。", "patch": "在死守数据合规红线的同时，面对狂飙突进的创新产品，试着用“如何让它合规地上架”来指导业务。", "skills": ["【被动】绝对法典映射", "【护盾】程序正义壁垒", "【反击】致命风险阻断"], "artifact": "【守夜人的封印法槌】", "base_roi": 1.15, "volatility": 0.10, "market_style": "绝对风险厌恶与固收类稳健策略", "evolution_path": ["L1 审查风控官", "L2 规则执行官"], "ultimate_evolution": "【绝对防御堡垒】全国一体化数据市场要素流转网络最终守门人"},
    "ESTJ": {"role": "核心业务统筹官", "tier": "SSR", "tier_color": "#ffd700", "rarity": "Top 8.7%", "base_hash": 9500, "desc": "无可争议的推进器，擅长将国家宏观政策拆解为团队可绝对执行的 KPI 矩阵。", "tags": ["强效统帅", "结果导向", "流程大师"], "partner": "ISTJ", "partner_advice": "让 ISTJ 担任您的品控质检员，在突发高并发危机时，直接将排障指挥权临时移交给 ISTP。", "tasks": ["发起并统筹 SDE 年度交易额破局百亿攻坚战", "强力调度跨部门资源打通确权交易清算堵点"], "black_swan": "KPI压倒一切导致“团队算力过载”。强推项目时易忽视一线团队的情绪阈值，引发内耗。", "patch": "在下发高压任务指令时，适度向团队释放“情绪价值”。具备高信任感的团队往往比单纯的数字目标走得更稳健。", "skills": ["【被动】战略降维拆解", "【战吼】全域资源狂暴", "【光环】铁腕意志压制"], "artifact": "【统帅的王权号角】", "base_roi": 1.35, "volatility": 0.25, "market_style": "动量突破与大容量核心资产配置", "evolution_path": ["L1 战区指挥官", "L2 跨域推进者"], "ultimate_evolution": "【全域秩序引擎】宏观数据业务推进的心脏中枢"},
    "INFJ": {"role": "产业生态智囊", "tier": "UR", "tier_color": "#ff003c", "rarity": "Top 0.9%", "base_hash": 9200, "desc": "具备极强的跨频段共情能力，能精准预判数据流转对未来实体经济产生的深远变革。", "tags": ["远见卓识", "使命驱动", "战略前瞻"], "partner": "ENFJ", "partner_advice": "将您的远见卓识交由 ENFJ 在核心圈层构建共识，让 ENFP 作为布道火种将您的理念扩散至全行业。", "tasks": ["规划 SDE 未来五年在实体经济的数据赋能版图", "发起“数据向善”及社会公益数据要素流通倡议"], "black_swan": "强烈的战略直觉若缺乏硬核量化数据支撑，向实干型领导汇报时极易被贴上“不切实际”标签。", "patch": "学会用精确的财务测算、合规条文来锚定您的宏大产业愿景。将“先知直觉”转化为具体的业务政策专报。", "skills": ["【被动】未来脉络洞视", "【光环】跨频灵魂共振", "【法术】精神信仰织网"], "artifact": "【预言家的全息星盘】", "base_roi": 1.55, "volatility": 0.35, "market_style": "宏观周期预判与长线价值发现", "evolution_path": ["L1 行业分析师", "L2 战略规划官"], "ultimate_evolution": "【全域生态先知】主导数字经济时代的底层精神共识"},
    "INFP": {"role": "生态价值主张官", "tier": "SR", "tier_color": "#a855f7", "rarity": "Top 4.4%", "base_hash": 8650, "desc": "冷酷数据背后的灵魂捕捉者，擅长在机械的交易网络中注入引人共鸣的生态文化。", "tags": ["价值感召", "组织粘合", "品牌定调"], "partner": "ENFJ", "partner_advice": "依托 ENFJ 的手腕在跨部门博弈中为您护航，联合 ISFP 将抽象的文化主张具象化为绝美的视觉产品。", "tasks": ["重塑 SDE 在数据交易领域的全球品牌叙事", "实施内部文化与跨部门协作协同凝聚力工程"], "black_swan": "在跨部门冷酷的算力与预算博弈中，容易因厌恶冲突而退缩，导致核心价值观无法落地。", "patch": "在跨部门协同博弈中，学会熟练利用预算工具和业务导向来捍卫您的核心价值主张，将柔性文化转化为硬性资产。", "skills": ["【被动】无形文化塑形", "【法术】直击灵魂叙事", "【光环】隐性品牌暴击"], "artifact": "【捕梦者的共振棱镜】", "base_roi": 1.25, "volatility": 0.30, "market_style": "ESG 价值投资与利基市场长尾策略", "evolution_path": ["L1 体验叙事者", "L2 品牌调性官"], "ultimate_evolution": "【灵魂共振奇核】赋予极客冷数据极其昂贵的品牌溢价"},
    "ENTJ": {"role": "战略开拓领军人", "tier": "UR", "tier_color": "#ff003c", "rarity": "Top 1.8%", "base_hash": 9900, "desc": "天生的矩阵建设者，在数据跨境、公共数据授权等探索区中展现极强的破局能力。", "tags": ["开疆拓土", "战略铁腕", "极致破局"], "partner": "INTJ", "partner_advice": "冲锋时将后背交给 INTJ 进行战略兜底，遇到底层技术阻击时，立刻呼叫 ISTP 进行定点爆破。", "tasks": ["主导“公共数据授权运营”省级破冰与资源抢占", "制定并执行跨链互认及全国数据大市场吞并战略"], "black_swan": "狂飙突进时的风控盲区。在极速吞并外部资源时极易因忽视底层合规红线而触发监管熔断。", "patch": "在极速开疆拓土时，请时刻保持与合规团队的数据同步。有时放慢半拍听听风控预警，能避开系统性风险。", "skills": ["【被动】绝对铁腕破局", "【战技】全域版图吞并", "【奥义】维度打击风暴"], "artifact": "【霸主的矩阵权杖】", "base_roi": 1.70, "volatility": 0.55, "market_style": "杠杆并购、特殊机会与高举高打", "evolution_path": ["L1 开拓先锋", "L2 战区统帅"], "ultimate_evolution": "【无界版图霸主】全国一体化数据市场的超级统帅"},
    "ENTP": {"role": "模式创新顾问", "tier": "SSR", "tier_color": "#ffd700", "rarity": "Top 3.2%", "base_hash": 9400, "desc": "传统交易规则的敏锐挑战者，致力于通过跨界思维寻找下一代数据要素流转范式。", "tags": ["范式重构", "逻辑激辩", "思维跳跃"], "partner": "INTP", "partner_advice": "把您天马行空的狂野点子丢给 INTP 进行逻辑降维，并指挥 ESTP 去市场上快速收割第一波红利。", "tasks": ["研发首个基于 Web3 的新型数据要素凭证通证", "在监管沙盒内主导蓝海型数字产品变现测试"], "black_swan": "无限发散思维导致的交付烂尾。若缺乏强力的落地跟进节点，极易沦为纯粹的纸上谈兵。", "patch": "适当收敛发散思维，选择一个极具潜力的创新点（如特定行业数据凭证），深度闭环跟进至最终交付。", "skills": ["【被动】旧有范式重塑", "【法术】跨界降维骇入", "【光环】次元逻辑破壁"], "artifact": "【欺诈师的混沌骰子】", "base_roi": 1.65, "volatility": 0.60, "market_style": "风险套利、期权重组与颠覆性投资", "evolution_path": ["L1 沙盒破坏者", "L2 跨界重组官"], "ultimate_evolution": "【范式秩序破坏者】亲手定义下个十年的交易元规则"},
    "ENFJ": {"role": "数商生态总监", "tier": "SSR", "tier_color": "#ffd700", "rarity": "Top 2.5%", "base_hash": 9350, "desc": "数据交易所的枢纽中心，能通过卓越的共识构建能力将多方利益竞争者聚拢为盟友。", "tags": ["关系枢纽", "温情领导力", "利益协同"], "partner": "INFJ", "partner_advice": "汲取 INFJ 的深度产业洞察作为布道弹药，并由 ESFJ 将您的宏观共识转化为具体的客情跟进单。", "tasks": ["构建辐射全国的 SDE 头部数商与第三方服务联盟", "维稳数据要素多边市场，调解核心生态伙伴冲突"], "black_swan": "对生态伙伴过度包容。处理违规事件时容易被“人情”裹挟，从而损害交易所的绝对中立性。", "patch": "在协调多方利益分配时，大胆引入客观的量化算法与智能合约刚性指标，确保“生态和谐”建立在规则基石之上。", "skills": ["【被动】绝对共识结盟", "【光环】极客温情统御", "【战技】利益杠杆平衡"], "artifact": "【枢纽的盟约契约】", "base_roi": 1.40, "volatility": 0.25, "market_style": "庞大资产池宏观调配与网络效应增强", "evolution_path": ["L1 渠道统筹", "L2 联盟主理人"], "ultimate_evolution": "【共识引力波】垄断全国超头数据商的绝对心智"},
    "ENFP": {"role": "平台资源布道大使", "tier": "SR", "tier_color": "#a855f7", "rarity": "Top 8.1%", "base_hash": 8900, "desc": "充满感染力的生态火苗，让每一场路演与推介都变成数据要素市场的狂热共识。", "tags": ["无限创意", "跨界纽带", "高频驱动"], "partner": "INFJ", "partner_advice": "在您的创意发散即将失控时，请务必听从 INFJ 的战略纠偏，并强迫自己遵循 INTJ 设定的节点框架。", "tasks": ["领衔 SDE 全国核心城市业务路演与生态宣发大循环", "策划并主持面向千家数商的“数据赋能创新工坊”"], "black_swan": "缺乏结构化数据追踪。路演现场火热但无法转化为 CRM 里的真实入驻率，商业核算价值打折。", "patch": "引入严密的商机日程表与里程碑管理。将您天马行空的生态创意转化为可追踪的业务转化漏斗。", "skills": ["【被动】群体情绪煽动", "【法术】全域流量黑洞", "【光环】异构资源嫁接"], "artifact": "【布道者的炽热火种】", "base_roi": 1.40, "volatility": 0.40, "market_style": "高波动趋势追逐与注意力经济炒作", "evolution_path": ["L1 宣发先锋官", "L2 流量矩阵中枢"], "ultimate_evolution": "【无界传播基站】把控国家全域要素市场的流量高地"},
    "ISFJ": {"role": "清结算运营中枢", "tier": "R", "tier_color": "#3b82f6", "rarity": "Top 13.8%", "base_hash": 8200, "desc": "最坚韧的底层支点，通过极致纠错与细节控场支撑起整个平台的专业信誉与高吞吐。", "tags": ["极致支撑", "服务巅峰", "高可用节点"], "partner": "ESFJ", "partner_advice": "将对外的复杂客情交由 ESFJ 抵挡，您只需与 ISTJ 背靠背，打造坚不可摧的后方清结算堡垒。", "tasks": ["保障全天候撮合及大额资金清结算体系 0 宕机", "极速响应并闭环处理生态节点与数商的底层工单"], "black_swan": "默默承受过载的技术债。不善于向上抗议，可能在交易洪峰期因人工审核量爆表而面临崩溃。", "patch": "在完美支撑中后台运转之余，尝试主动提出冗余流程的优化提案。您的实操痛点极具价值。", "skills": ["【被动】极限并发支撑", "【法术】毫米级量子纠错", "【护盾】最后绝对防线"], "artifact": "【守卫者的永动齿轮】", "base_roi": 1.10, "volatility": 0.08, "market_style": "极低回撤避险与无风险极致套利策略", "evolution_path": ["L1 运营专员", "L2 平台质检官"], "ultimate_evolution": "【绝对永动节点】维持交易所生命线的最终坚盾"},
    "ESFJ": {"role": "政企商务枢纽", "tier": "SR", "tier_color": "#a855f7", "rarity": "Top 12.3%", "base_hash": 8750, "desc": "超级连接器，擅长经营多维度的外部政企生态关系，是前线业务部门的最强润滑剂。", "tags": ["协作典范", "客情控制", "社会化支撑"], "partner": "ISFJ", "partner_advice": "在前线维护八方客情时，把繁杂的合同流转放心抛给 ISFJ，遇到合规红线问题立刻请 ESTJ 强硬回绝。", "tasks": ["高频维护国家部委及地方大数据局的核心 G 端客情", "统筹落地具有全国影响力的年度数据要素高峰论坛"], "black_swan": "过度满足多方诉求导致的边界失守。极易因“谁都想讨好”而签下严重偏离平台底线的协议。", "patch": "在维护复杂商务生态时，建立更独立的合规风险过滤网。在照顾合作方诉求时保持底线清醒。", "skills": ["【被动】政企超级链接", "【光环】社会化缓冲巨网", "【法术】多方负载均衡"], "artifact": "【外交官的破冰金卡】", "base_roi": 1.20, "volatility": 0.15, "market_style": "庞大资金盘稳健配置与政企引导基金模式", "evolution_path": ["L1 商务专员", "L2 政企主理"], "ultimate_evolution": "【政企超导桥梁】构筑不可替代的 G 端业务护城河"},
    "ISTP": {"role": "平台风控与排障专家", "tier": "SR", "tier_color": "#a855f7", "rarity": "Top 5.4%", "base_hash": 8950, "desc": "数据底座的实干派，只对事实和逻辑代码负责，是系统面临大并发技术故障时的定海神针。", "tags": ["极简实干", "故障排查", "硬核运维"], "partner": "ESTP", "partner_advice": "当 ESTP 在前线疯狂接单导致系统过载时，由您负责底层扩容，并与 INTP 联手重构最优并发算法。", "tasks": ["执行 SDE 核心交易链路的灾备拉起与物理级排障", "在不影响前台撮合的前提下执行底层架构高危热更新"], "black_swan": "技术彻底黑盒化。过度依赖个人的“极客直觉”排障，一旦休假离线会导致整个系统应急瘫痪。", "patch": "尝试将您极度内隐的底层排查经验，沉淀为可视化的《应急响应标准手册》。打破沟通壁垒。", "skills": ["【被动】致命物理拔线", "【法术】量子黑盒破解", "【光环】超体极客直觉"], "artifact": "【机修师的万能扳手】", "base_roi": 1.35, "volatility": 0.35, "market_style": "黑天鹅事件驱动、困境反转与系统级技术套利", "evolution_path": ["L1 底层架构师", "L2 灾备指挥官"], "ultimate_evolution": "【底层代码幽灵】掌控国家要素机房的绝对生命力"},
    "ISFP": {"role": "资产交互体验官", "tier": "R", "tier_color": "#3b82f6", "rarity": "Top 8.8%", "base_hash": 8150, "desc": "赋予枯燥数据以美学权重，致力于提升数据产品在终端大屏展示时的绝对视觉专业质感。", "tags": ["审美溢价", "感官叙事", "体验极致"], "partner": "ESFP", "partner_advice": "将您的美学产出交由 ESFP 在各大峰会上高调发布，并从 INFP 构筑的生态故事中汲取设计灵感。", "tasks": ["重构 SDE 实时交易大盘的动态数据全息视觉渲染", "主导面向数商终端的 UI/UX 操作流敏捷体验升级"], "black_swan": "陷入纯粹形式主义。设计出极其炫酷的大屏，却完全脱离了“数据确权撮合”的核心商业逻辑。", "patch": "在追求终端展示的美学溢价时，适度增加对核心确权流转逻辑和底层交易协议的理解。", "skills": ["【被动】沉浸感官叙事", "【法术】低维美学重构", "【光环】绝对心流捕获"], "artifact": "【幻像师的全息画笔】", "base_roi": 1.18, "volatility": 0.22, "market_style": "艺术品级别非标资产与另类情感投资估值", "evolution_path": ["L1 UI视觉专员", "L2 交互总监"], "ultimate_evolution": "【感官具象师】以一己之力拉升数据产品百倍视觉溢价"},
    "ESTP": {"role": "前沿敏捷先锋", "tier": "SSR", "tier_color": "#ffd700", "rarity": "Top 4.3%", "base_hash": 8800, "desc": "数据流通一线的敏锐猎手，能极快捕捉到瞬息万变的市场红利与应用空间的套利机会。", "tags": ["市场直觉", "敏捷收割", "实战专家"], "partner": "ISTP", "partner_advice": "尽情在市场前线厮杀套利，让 ISTP 为您搭建最稳固的技术跳板，遇到僵局迅速呼叫 ENTJ 支援。", "tasks": ["敏锐收割新政策出台后的第一波“短期数据流通红利”", "针对区域内竞所的市场抢夺发起极速实战反制突击"], "black_swan": "为了极速促成首单，倾向于利用捷径绕过繁琐的合规防火墙，一旦溯源出瑕疵将面临毁灭性反噬。", "patch": "在捕捉市场瞬时机遇、展现高效行动力促成交易时，务必将前置合规审查纳入操作流程中。", "skills": ["【被动】毫秒瞬时收割", "【战技】全图火力覆盖", "【光环】血腥红利嗅探"], "artifact": "【猎手的套利罗盘】", "base_roi": 1.50, "volatility": 0.50, "market_style": "超高频日内交易与极限突发利好嗜血收割", "evolution_path": ["L1 突击交易员", "L2 战地狼王"], "ultimate_evolution": "【极速套利猎手】全网数据交易套利空间的绝杀狙击者"},
    "ESFP": {"role": "官方品牌发声信标", "tier": "SR", "tier_color": "#a855f7", "rarity": "Top 9.9%", "base_hash": 8300, "desc": "交易所的前台形象窗口，天生具备将复杂的政策解码为大众传播话术的超级天赋。", "tags": ["全域表现", "舆情响应", "公关信标"], "partner": "ISFP", "partner_advice": "用您极具感染力的表现为 ISFP 的视觉产品带货，并与 ENFP 联手策划能掀起全网狂潮的顶级路演。", "tasks": ["在全网引爆 SDE 最新明星数据产品的展会级宣发流量", "冲在第一线对冲平台突发的负面市场舆情并进行柔性公关"], "black_swan": "由于对底层条款理解深度不够，对外宣发时极易出现“用词越界”，引发监管舆情风险。", "patch": "花时间深潜研究数据要素的底层逻辑与政策红头文件。将您的绝佳表现力建立在扎实的产业根基上。", "skills": ["【被动】全域舆论控场", "【光环】情绪频率调控", "【奥义】暴走危机降维"], "artifact": "【扩音器的精神频段】", "base_roi": 1.35, "volatility": 0.38, "market_style": "舆论狂热驱动、社交媒体共振与情绪面交易", "evolution_path": ["L1 品牌代言", "L2 首席发言人"], "ultimate_evolution": "【极速情绪信标】左右资本市场情绪波动的首席发声端"}
}
...
if not st.session_state.started:
    ...
    data = mbti_details.get(mbti, mbti_details["INTJ"]) # Safe fallback
    ...
    # 🔐 资产防篡改物理锁：强制覆盖旧版本缓存，彻底杜绝 KeyError 和 NameError
    if "asset_minted" not in st.session_state or st.session_state.asset_minted.get("version") != VERSION:
        ...
        d_arr_gen, roi_arr_gen = generate_alpha_curve(data.get('base_roi', 1.35), data.get('volatility', 0.35), int(h_code[:6], 16))
        st.session_state.asset_minted = {
            "version": VERSION,
            "hash_code": h_code,
            "block_height": f"V7.0-{(int(time.time()) % 1000000):06d}",
    ...
    hash_code = asset.get("hash_code", "0x00000000")
    block_height = asset.get("block_height", "V7.0-000000")
    current_time_str = asset.get("time_str", "Unknown")
    token_id = asset.get("token_id", 0)
    contract_addr = asset.get("contract_addr", "0x00000000")
    ...
    with col_top_r:
        ...
        st.markdown("<div style='background:rgba(10, 15, 25, 0.8); border:1px solid rgba(255,255,255,0.1); border-radius:12px; padding:clamp(15px, 4vw, 25px); box-shadow:inset 0 0 20px rgba(0,0,0,0.5); margin-bottom:20px;'><h4 style='color:#3b82f6 !important; border-left:4px solid #3b82f6; padding-left:10px; font-weight:900; margin-bottom:15px;'>🤝 羁绊连携阵列 (SYNERGY)</h4><div style='font-size:13px; color:#94a3b8; margin-bottom:15px;'>输入同盟卡牌代码，触发双轨共鸣与连携特效：</div>", unsafe_allow_html=True)
    ...
    with col_mid_r:
        ...
        st.markdown(f"<div style='text-align:center; font-size:16px; font-weight:bold; color:{r_color}; font-family:Noto Sans SC; margin-top: 5px;'>风控倾向：{r_tag}</div>", unsafe_allow_html=True)
        ...
        st.plotly_chart(get_3d_topology(res, val_E, val_I, val_S, val_N, val_T, val_F, mbti, tier_color, h_int), use_container_width=True, config={'displayModeBar': False}, theme=None)

    # =========================================================================
    # 📱【第三排：赛博魔典深潜面板 (Tabs)】
    # =========================================================================
    st.markdown("<h4 style='color:#ffd700 !important; border-left:4px solid #ffd700; padding-left:10px; font-weight:900; margin-top:30px; margin-bottom:15px;'>🗄️ 赛博魔典档案 (GRIMOIRE)</h4><div style='text-align:right; font-size:12px; color:#94a3b8; margin-bottom:10px; opacity:0.8;'>👉 手机端可左右滑动切换面板</div>", unsafe_allow_html=True)
    t_evo, t_mkt, t_sol, t_bids = st.tabs(["🤖 神谕演进", "📉 斗技 Alpha", "💻 铸造合约", "📊 撮合盘口"])
    
    with t_evo:
        # 🤖 神级特性 1：AI 神谕打字机终端 (物理隔离 CSS 防止污染)
    ...
    with t_mkt:
        ...
        # 排位榜
        st.markdown(get_leaderboard_html(safe_alias_final, asset_valuation, tier_color, h_int).replace('\n', ''), unsafe_allow_html=True)
    ...
    with t_bids:
        ...
        st.markdown(BIDS_CSS.replace('\n', ''), unsafe_allow_html=True)
        HTML_BIDS = f"""<div style="background: #050505; border: 1px solid #334155; border-radius: 8px; padding: 20px; font-family: 'Fira Code', monospace; font-size: 12px; color: #94a3b8; box-shadow: inset 0 0 20px rgba(0,0,0,0.8);">{depth_html}<div style="display: flex; justify-content: space-between; border-bottom: 1px dashed #334155; padding-bottom: 12px; margin-bottom: 15px; color: #e2e8f0; font-weight: bold; font-size: 11px; letter-spacing: 1px;"><span style="width:40%;">[INSTITUTION]</span><span style="width:30%; text-align:center;">[BID_SIZE]</span><span style="width:30%; text-align:right;">[PREMIUM]</span></div>{bids_html}<div style="text-align: center; margin-top: 15px; font-size: 10px; color: #3b82f6; animation: blink 1.5s infinite;">● WAITING FOR NEW BIDS...</div></div>"""
    ...
    with t_img:
        ...
        st.markdown("<div style='font-size:13px; color:#10b981; margin-bottom:15px; line-height:1.7;'>已为您渲染【可见即所得】高清卡砖。如果下方「保存按钮」无反应，<b style='color:#ffd700; font-size:15px;'>您可以直接使用手机系统截屏保存！</b>绝对不再死锁！</div>", unsafe_allow_html=True)
        ...
        # 绝对不使用外部网络字体，完全脱离 Markdown
        POSTER_CSS = """<style>body { margin: 0; padding: 10px 0; background: transparent !important; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; color: #fff; overflow-x: hidden; text-align: center; }.capture-box { width: 320px; background-color: #010308; padding: 15px; border-radius: 12px; border: 3px solid #cbd5e1; box-shadow: 0 0 25px rgba(0, 243, 255, 0.4); position: relative; overflow: hidden; margin: 0 auto; text-align: left; box-sizing: border-box; }.cyber-grid { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-image: linear-gradient(0deg, rgba(0,243,255,0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(0,243,255,0.05) 1px, transparent 1px); background-size: 25px 25px; z-index: 0; pointer-events:none;}.psa-header { background: #ef4444; padding: 10px; border-radius: 6px; display: flex; justify-content: space-between; align-items: center; border: 1px solid #b91c1c; margin-bottom: 15px; position: relative; z-index: 2; box-shadow: inset 0 0 10px rgba(0,0,0,0.5);}.psa-grade { font-size: 28px; font-weight: 900; color: #fff; line-height: 1; font-family: Impact, sans-serif; }.psa-desc { font-size: 10px; color: #fecaca; text-align: right; font-weight: bold; line-height: 1.2; text-transform: uppercase; }.inner-card { background: rgba(10,15,30,0.95); border: 2px solid rgba(255,215,0,0.4); border-radius: 8px; padding: 20px 15px; position: relative; z-index: 2; overflow: hidden;}.hd { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px dashed rgba(0,243,255,0.3); padding-bottom: 10px; margin-bottom: 15px; }.nm { text-align: center; font-size: 18px; font-weight: 900; letter-spacing: 2px; margin-bottom: 5px; color: #fff; text-transform: uppercase; }.mb { font-size: 52px; font-weight: 900; color: #ffd700; line-height: 1; text-align: center; text-shadow: 0 0 30px rgba(255,215,0,0.6); margin-bottom: 5px; letter-spacing: 4px; font-family: Impact, sans-serif; }.rl { text-align: center; font-size: 13px; font-weight: 900; color: #00f3ff; margin-bottom: 15px; letter-spacing: 2px; }.vb { display:flex; justify-content:space-between; text-align:center; background:rgba(0,0,0,0.8); border:1px solid rgba(255,215,0,0.4); padding:10px; border-radius:8px; margin-bottom: 15px; gap: 5px; }.ft { text-align: center; color: #64748b; font-size: 8px; padding-top: 10px; line-height: 1.6; font-family: monospace; position: relative; z-index: 2; margin-top: 10px;}.stat-row { display: flex; align-items: center; margin-bottom: 8px; font-size: 10px; font-weight: bold; justify-content: space-between; }.sbc { background: rgba(255,255,255,0.05); border-radius: 3px; height: 5px; width: 130px; position: relative; overflow: hidden; margin: 0 6px; }.sbf { position: absolute; left: 0; top: 0; height: 100%; }.action-btn { background: linear-gradient(90deg, #10b981, #059669); color: #fff; border: none; padding: 15px 30px; border-radius: 8px; font-weight: bold; font-size: 15px; cursor: pointer; box-shadow: 0 5px 15px rgba(16,185,129,0.4); margin-top: 20px; text-transform: uppercase; width: 320px; transition: all 0.2s ease;} .action-btn:active { transform: scale(0.98); } #result-img { display: none; width: 320px; border-radius: 12px; margin: 0 auto; box-shadow: 0 0 30px rgba(16,185,129,0.6); }.success-msg { display: none; color: #10b981; font-weight: bold; margin-top: 15px; font-size: 14px; background: rgba(16,185,129,0.1); padding: 10px; border-radius: 6px; border: 1px solid #10b981; width: 320px; margin: 15px auto 0 auto; box-sizing: border-box; }</style>"""

        HTML_POSTER_BODY = f"""<div id="html-card"><div class="capture-box" id="capture-target"><div class="cyber-grid"></div><div class="psa-header"><div><div style="font-size:12px; font-weight:900; color:#fff; letter-spacing:1px; margin-bottom:2px;">SDE AUTHENTICATED</div><div style="font-size:9px; color:#fca5a5; font-family:monospace;">CERT: {hash_code[:10]}</div></div><div style="display:flex; align-items:center; gap:10px;"><div class="psa-desc">GEM<br>MINT</div><div class="psa-grade">10</div></div></div><div class="inner-card"><div style="position: absolute; top: 15px; right: -35px; background: {tier_color}; color: #000; font-weight: 900; font-size: 10px; padding: 3px 35px; transform: rotate(45deg); z-index: 10; letter-spacing: 2px; box-shadow: 0 0 15px {tier_color}88;">{tier_level}</div><div class="hd"><div style="font-size:13px;font-weight:900; color:#00f3ff; letter-spacing:1px;">GENESIS TCG</div><div style="font-size:9px;color:#94a3b8; font-weight:bold;">2026 EDITION</div></div><div style="font-size:9px;color:#94a3b8;text-align:center;margin-bottom:5px;">NODE IDENTIFIER</div><div class="nm">{full_title}</div><div class="mb">{mbti}</div><div style="text-align:center;font-size:10px;color:#94a3b8;margin-bottom:15px; font-weight:bold;">RARITY: <span style="color:{tier_color};font-size:12px;">{data.get('rarity', 'Top 5%')}</span></div><div class="rl">【 {role_name} 】</div><div class="vb"><div style="flex:1;"><div style="font-size:9px;color:#94a3b8;margin-bottom:5px;">COMBAT POWER</div><div style="font-size:16px;color:#ffd700;font-weight:900;">{round(asset_valuation, -4):,}</div></div><div style="border-left:1px dashed rgba(255,255,255,0.3);"></div><div style="flex:1;"><div style="font-size:9px;color:#94a3b8;margin-bottom:5px;">PERCENTILE</div><div style="font-size:16px;color:#00f3ff;font-weight:900;">TOP {100-pct_beat:.1f}%</div></div></div><div style="text-align:center; margin-bottom:12px;"><div style="font-size:10px; color:#a855f7; margin-bottom:6px; font-weight:bold;">[ ABILITY MOVES ]</div>{skills_html_poster}</div><div style="text-align:center; margin-bottom:15px;">{tags_html_poster}</div><div style="background: rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; padding: 12px 10px; margin-bottom: 10px;"><div style="font-size: 8px; color: #00f3ff; text-align: center; margin-bottom: 8px; font-family: monospace;">/// BASE STATS ///</div><div class="stat-row"><span style="color:#e2e8f0; width:35px;">输出</span><div class="sbc"><div class="sbf" style="width:{val_E}%; background:#00f3ff;"></div></div><span style="color:#94a3b8; width:35px; text-align:right;">隐匿</span></div><div class="stat-row"><span style="color:#e2e8f0; width:35px;">精准</span><div class="sbc"><div class="sbf" style="width:{val_S}%; background:#a855f7;"></div></div><span style="color:#94a3b8; width:35px; text-align:right;">视界</span></div><div class="stat-row"><span style="color:#e2e8f0; width:35px;">护甲</span><div class="sbc"><div class="sbf" style="width:{val_T}%; background:#3b82f6;"></div></div><span style="color:#94a3b8; width:35px; text-align:right;">光环</span></div><div class="stat-row" style="margin-bottom:0;"><span style="color:#e2e8f0; width:35px;">秩序</span><div class="sbc"><div class="sbf" style="width:{val_J}%; background:#10b981;"></div></div><span style="color:#94a3b8; width:35px; text-align:right;">敏捷</span></div></div><div style="font-size:11px; font-weight:bold; color:#10b981; text-align:center; border-top:1px dashed #334155; padding-top:10px;">遗物装备：<span style="color:#ffd700;">{artifact}</span></div></div><div class="ft"><div style="width: 90%; height: 20px; margin: 0 auto 8px auto; background: {barcode_css};"></div><div style="margin-bottom:2px;font-weight:bold;">SDE CYBER-NODE TCG V7.0</div><div style="color:#475569;">© {COPYRIGHT} | ID: #{token_id}</div></div></div><button class="action-btn" id="btn-render" onclick="execRender()">📸 点击生成为高清图片</button></div><img id="result-img" alt="SDE Matrix Slab" title="长按保存或分享" /><div id="success-msg" class="success-msg">✅ <b>图片压制成功！</b><br>👆 手机端请长按上方图片保存发圈。</div>"""

        JS_SCRIPT = """<script>function execRender() { var btn = document.getElementById('btn-render'); btn.innerHTML = "⏳ 正在压制，请稍候..."; btn.style.opacity = "0.7"; if(typeof html2canvas === 'undefined') { btn.innerHTML = "❌ 环境不支持，请直接截屏上图"; btn.style.opacity = "1"; return; } setTimeout(function() { html2canvas(document.getElementById('capture-target'), { scale: 2, backgroundColor: '#010308', useCORS: true, logging: false }).then(function(canvas) { document.getElementById('result-img').src = canvas.toDataURL('image/png'); document.getElementById('result-img').style.display = 'block'; document.getElementById('html-card').style.display = 'none'; document.getElementById('success-msg').style.display = 'block'; }).catch(function(e) { btn.innerHTML = "❌ 生成失败，请直接手机截屏"; btn.style.opacity = "1"; }); }, 200); }</script>"""
        
        st.components.v1.html(f"<!DOCTYPE html><html><head><meta charset='utf-8'><script src='https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js'></script>{POSTER_CSS.replace(chr(10), '')}</head><body>{HTML_POSTER_BODY.replace(chr(10), '')}{JS_SCRIPT.replace(chr(10), '')}</body></html>", height=950)
