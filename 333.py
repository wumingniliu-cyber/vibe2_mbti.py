...
# 🚨 终极防爆工具：强行剥离所有前置空格，杜绝 Streamlit 错误识别为代码块！
def render_html(html_str):
    cleaned = '\n'.join([line.lstrip() for line in html_str.split('\n') if line.strip() != ''])
    st.markdown(cleaned, unsafe_allow_html=True)

# ==============================================================================
# 🎨 [ CORE 02 ] 赛博全息 UI 渲染底座 (安全隔离与响应式优化)
# ==============================================================================
st.markdown("""
<style>
...
def trigger_cyber_activation():
    render_html("""
    <style>
        .cyber-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: #030712; z-index: 999999; display: flex; justify-content: center; align-items: center; flex-direction: column; animation: cyber-fadeout 1.8s cubic-bezier(0.8, 0, 0.2, 1) forwards; pointer-events: none; }
        .cyber-scanner { width: 100%; height: 4px; background: #00f3ff; box-shadow: 0 0 20px #00f3ff, 0 0 40px #00f3ff; position: absolute; top: 0; left: 0; animation: scan-down 1.2s ease-in-out forwards; }
        .cyber-text { font-family: 'Orbitron', sans-serif; font-size: clamp(24px, 5vw, 48px); font-weight: 900; color: #fff; letter-spacing: 10px; text-shadow: 0 0 20px #00f3ff; opacity: 0; animation: pop-in 1.2s cubic-bezier(0.1, 0.8, 0.3, 1) forwards; }
        @keyframes scan-down { 0% { transform: translate3d(0,0,0); opacity: 1; } 80% { transform: translate3d(0,100vh,0); opacity: 1; } 100% { transform: translate3d(0,100vh,0); opacity: 0; } }
        @keyframes pop-in { 0% { transform: scale3d(0.8,0.8,1); opacity: 0; } 20% { transform: scale3d(1.05,1.05,1); opacity: 1; } 80% { transform: scale3d(1,1,1); opacity: 1; } 100% { transform: scale3d(1.2,1.2,1); opacity: 0; } }
        @keyframes cyber-fadeout { 0%, 70% { opacity: 1; } 100% { opacity: 0; visibility: hidden; } }
    </style>
    <div class="cyber-overlay"><div class="cyber-scanner"></div><div class="cyber-text">ACCESS GRANTED</div></div>
    """)
...
# 🚨 18 维满血防爆字典：彻底补齐所有字典键值对，JSON 导出 0 报错！
mbti_details = {
    ...
    "ISTJ": {"role": "合规审查主理官", "tier": "SR", "tier_color": "#a855f7", "rarity": "Top 11.6%", "base_hash": 8850, "base_roi": 1.15, "volatility": 0.10, "market_style": "绝对风险厌恶与固收类稳健策略", "desc": "SDE 底层防线的守夜人，您的评估本身就是安全、严谨与业务零失误的代名词。", "tags": ["绝对合规", "程序正义", "风险兜底"], "skills": ["合规映射(Lv.Max)", "程序正义", "风险阻断"], "evolution_path": ["L1 审查风控官", "L2 规则执行官", "L3 绝对防御壁垒"], "ultimate_evolution": "【绝对防御壁垒】全国一体化数据市场要素流转网络的终极守门人", "partner": "ESTJ (业务推进节点)", "partner_advice": "配合 ESTJ 建立坚不可摧的业务流水线，形成最硬核交付闭环。", "tasks": ["主导头部数商“数据资源入表”全链路审计", "设计业务合同与智能合约的合规映射SOP"], "black_swan": "过度依赖既有 SOP。面临无先例创新业务时，容易因“无库可查”产生本能排斥与误杀。", "patch": "在死守数据合规红线的同时，面对创新产品，试着用“如何让它合规地上架”来指导业务。"},
    ...
    "ESFJ": {"role": "政企商务枢纽", "tier": "SR", "tier_color": "#a855f7", "rarity": "Top 12.3%", "base_hash": 8750, "base_roi": 1.20, "volatility": 0.15, "market_style": "庞大资金盘稳健配置与政企引导基金模式", "desc": "超级连接器，擅长经营多维度的外部政企生态关系，是前线业务部门的最强润滑剂。", "tags": ["协作典范", "客情控制", "社会化支撑"], "skills": ["超级链接(Lv.Max)", "社会化缓冲网", "负载均衡"], "evolution_path": ["L1 商务专员", "L2 政企主理", "L3 政企超导桥梁"], "ultimate_evolution": "【政企超导桥梁】构筑不可替代的 G 端业务护城河", "partner": "ESTJ (宏观决策节点)", "partner_advice": "前线维护八方客情时把繁杂合同流转抛给 ISFJ，遇到合规红线问题立刻请 ESTJ 强硬回绝。", "tasks": ["高频维护国家部委及地方大数据局的核心 G 端客情", "统筹落地具有全国影响力的年度数据要素高峰论坛"], "black_swan": "过度满足多方诉求导致边界失守。极易因“谁都想讨好”而签下严重偏离平台底线的协议。", "patch": "维护复杂商务生态时，建立更独立的合规风险过滤网。照顾合作方诉求时保持底线清醒。"},
    "ISTP": {"role": "平台风控与排障专家", "tier": "SR", "tier_color": "#a855f7", "rarity": "Top 5.4%", "base_hash": 8950, "base_roi": 1.35, "volatility": 0.35, "market_style": "事件驱动、困境反转与系统级技术套利", "desc": "数据底座的实干派，只对事实和逻辑代码负责，是大并发技术故障时的定海神针。", "tags": ["极简实干", "故障排查", "硬核运维"], "skills": ["物理拔线(Lv.Max)", "黑盒破解", "极客直觉"], "evolution_path": ["L1 底层架构师", "L2 灾备指挥官", "L3 底层代码幽灵"], "ultimate_evolution": "【底层代码幽灵】掌控国家要素机房的绝对生命力", "partner": "INTP (算法优化节点)", "partner_advice": "当 ESTP 在前线疯狂接单导致系统过载时由您负责扩容，并与 INTP 联手重构最优并发算法。", "tasks": ["执行 SDE 核心交易链路的灾备拉起与物理级排障", "在不影响前台撮合的前提下执行底层架构高危热更新"], "black_swan": "技术彻底黑盒化。过度依赖个人的极客直觉排障，一旦休假离线会导致整个系统应急瘫痪。", "patch": "尝试将极度内隐的底层排查经验，沉淀为可视化的《应急响应标准手册》，打破沟通壁垒。"},
...
if not st.session_state.started:
    ...
    with center_container():
        ...
        with st.form(key="login_form", border=False):
            render_html("<div style='color:#00f3ff; font-family:\"Orbitron\", sans-serif; font-size:12px; font-weight:bold; margin-bottom:8px; text-align:center;'>▼ MOUNT NODE IDENTIFIER (输入授权节点代号) ▼</div>")
    ...
    with center_container():
        render_html("<h2 class='hero-title' data-text='[ HASHING NEURAL NETWORK ]' style='font-size:28px !important; margin-top:50px; text-align:center; display:block;'>[ HASHING NEURAL NETWORK ]</h2>")
        mint_box = st.empty()
        h_logs = ""
        for _ in range(12):
    ...
    # 🚨 40题满血算法归一化 (每维10题，极值[-20, 20]，映射至 [0, 100])
    ...
    # 🔐 资产不可篡改锁：彻底解决下拉框重刷带来的哈希与 JSON NameError 崩溃 Bug
    ...
    col_top_l, col_top_r = st.columns([1, 1], gap="large")
    ...
    col_mid_l, col_mid_r = st.columns([1.1, 1.2], gap="large")
    ...
    with col_mid_r:
        ...
        with t_evo:
            ...
            <div style="color:#ffd700; font-family:Orbitron; font-size:11px; margin-bottom:2px; letter-spacing:1px;">PHASE 3 (ULTIMATE DOMINANCE)</div><div style="color:#ffd700; font-weight:900; font-size:18px;">{data['ultimate_evolution']}</div>
            </div>
            """)
