...
# ==============================================================================
# 🚨 必须顶格书写！绝对防止 Streamlit 把空格缩进识别为 Markdown 代码块！
global_css = """
<style>
...
# --- 🚀 满血复原：纯 GPU 硬件加速的量子脉冲扫描 (EMP) 解锁特效 (替代原本卡顿的 666) ---
def trigger_cyber_activation():
    html_str = """
<style>
.cyber-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(3,7,18,0.95); z-index: 999999; display: flex; justify-content: center; align-items: center; flex-direction: column; animation: cyber-fadeout 2.8s cubic-bezier(0.8, 0, 0.2, 1) forwards; pointer-events: none; backdrop-filter: blur(5px); }
.matrix-code { position: absolute; top: 40%; left: 50%; transform: translate(-50%, -50%); color: #10b981; font-family: monospace; font-size: 16px; text-align: center; line-height: 1.8; opacity: 0; animation: matrix-glitch 2s forwards; white-space: pre; text-shadow: 0 0 10px #10b981;}
...
# 🚨 18 维满血防爆字典 (无损保留所有 txt 文本描述，彻底杜绝 KeyError)
mbti_details = {
    "INTJ": {
    ...
    "INFP": {
        ...
        "skills": ["文化塑形(Lv.Max)", "灵魂叙事", "隐性品牌溢价"],
    ...
    "ISFJ": {
        ...
        "skills": ["极限并发支撑(Lv.Max)", "毫米级纠错", "绝对防线"],
    ...
    "ESFJ": {
        ...
        "skills": ["超级链接(Lv.Max)", "社会化缓冲网", "负载均衡"],
...
if not st.session_state.started:
    ...
    if not st.session_state.boot_played:
        html_boot = """
<style>
.sys-boot-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: #030712; z-index: 9999999; display: flex; justify-content: center; align-items: center; flex-direction: column; animation: sys-boot-fade 2.5s 1.5s cubic-bezier(0.8, 0, 0.2, 1) forwards; pointer-events: none; }
.sys-boot-logo { color: #00f3ff; font-family: 'Orbitron', monospace; font-size: 24px; font-weight: 900; letter-spacing: 4px; overflow: hidden; border-right: 3px solid #00f3ff; white-space: nowrap; animation: typing-boot 1.2s steps(20, end) forwards, blink-boot 0.5s step-end infinite; }
.sys-boot-bar-bg { width: 260px; height: 2px; background: rgba(0,243,255,0.1); margin-top: 20px; position: relative; }
.sys-boot-bar-fill { position: absolute; top: 0; left: 0; height: 100%; background: #00f3ff; box-shadow: 0 0 15px #00f3ff; animation: load-boot 1.5s ease-out forwards; }
...
"""
    ...
    # 🔐 资产防篡改物理锁：彻底解决下拉框重刷带来的哈希与 JSON NameError 崩溃 Bug
...
""")
        ...
        with t_sol:
            ...
            # 🚨 终极杀手锏：彻底抛弃会被主题污染的 st.code，直接使用纯 HTML 画代码黑盒！永不白底白字！
...
contract SDE_Talent_Registry_V1 is ERC721 {{
    ...
    function executeMint() public {{
        ...
        _mint(msg.sender, tokenId);
            ...
            html_solidity = f"""
<div style="background-color: #050505 !important; border: 1px solid #334155 !important; border-left: 4px solid #10b981 !important; border-radius: 8px; padding: 20px; overflow-x: auto; margin-bottom: 20px; margin-top: 10px; box-shadow: inset 0 0 20px rgba(0,0,0,0.8);">
<pre style="margin: 0; font-family: 'Fira Code', monospace; font-size: 12px; color: #10b981 !important; line-height: 1.6; text-shadow: 0 0 5px rgba(16,185,129,0.3); background: transparent; border: none;"><code>{safe_code}</code></pre>
</div>
"""
...
.bdg {{ position: absolute; top: 22px; right: -35px; background: {tier_color}; color: #000; font-family: 'Orbitron'; font-weight: 900; font-size: 10px; padding: 3px 35px; transform: rotate(45deg); z-index: 10; letter-spacing: 2px; box-shadow: 0 0 15px {tier_color}88; }}
...
// 🚨 优雅降级：防止部分旧内核不支持 document.fonts 引发崩溃
if (document.fonts && document.fonts.ready) {{
document.fonts.ready.then(executeRender).catch(executeRender);
}} else {{
executeRender();
}}
</script>
...
"""
        components.html(html_to_image_script, height=1100)
...
🔗 [Token ID: #{token_id} | Hash: 0x{hash_code[:8]}]"""
    ...
    with t_json:
        ...
        # 🚨 终极免疫 JSON 导出 NameError！闭环所有变量读取
        export_data = {
            "version": VERSION,
