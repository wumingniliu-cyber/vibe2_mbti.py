...
# 🚨 终极防爆工具：强行剥离所有前置特殊空格，物理杜绝 Streamlit 错误识别为 Markdown 代码块！
def render_html(html_str):
    cleaned = '\n'.join([line.lstrip() for line in html_str.split('\n')])
    st.markdown(cleaned, unsafe_allow_html=True)

# ==============================================================================
# 🎨 [ CORE 02 ] 赛博全息 UI 渲染底座 (完整展开，拒绝压缩)
# ==============================================================================
render_html("""
<style>
    ...
    [data-testid="stAppViewContainer"]::before { 
        ...
        background: radial-gradient(circle at 50% 50%, rgba(0, 243, 255, 0.05) 0%, rgba(3, 7, 18, 1) 70%); 
    ...
    /* 保护原生组件字色 */
    .stMarkdown, p, span, h2, h3, h4, li, div, label { 
        color: #f8fafc !important; 
        z-index: 2; 
        position: relative; 
    }
    [data-testid="stProgress"] > div > div > div { 
        background: linear-gradient(90deg, #00f3ff, #3b82f6) !important; 
        box-shadow: 0 0 15px rgba(0,243,255,0.8); 
    }

    /* 硬件加速顶部大盘流 */
    .ticker-wrap { 
        width: 100vw; overflow: hidden; height: 32px; 
        background-color: rgba(2, 6, 23, 0.98); 
        border-bottom: 1px solid rgba(0,243,255,0.4); 
        position: fixed; top: 0; left: 0; z-index: 99990; 
        box-shadow: 0 2px 15px rgba(0,243,255,0.15); 
    }
    .ticker { 
        display: inline-block; white-space: nowrap; padding-right: 100%; box-sizing: content-box; 
        animation: ticker 40s linear infinite; 
        font-family: 'Orbitron', monospace; font-size: 12px; color: #00f3ff; line-height: 32px; 
        letter-spacing: 1px; will-change: transform; 
    }
    .ticker span { margin-right: 50px; } 
    .ticker .up { color: #10b981; text-shadow: 0 0 5px rgba(16,185,129,0.5); } 
    .ticker .down { color: #f43f5e; text-shadow: 0 0 5px rgba(244,63,94,0.5); }
    @keyframes ticker { 0% { transform: translate3d(0, 0, 0); } 100% { transform: translate3d(-100%, 0, 0); } }

    /* 骇客主标题 */
    .hero-title { 
        font-size: clamp(30px, 5vw, 42px) !important; 
        font-weight: 900 !important; text-align: center; color: #ffffff !important; 
    ...
    /* 模块大标题 */
    .module-title { 
        color: #00f3ff !important; border-left: 5px solid #00f3ff; padding-left: 12px; 
        font-weight: 900; margin-top: 45px; margin-bottom: 20px; letter-spacing: 1px; 
        font-family: 'Noto Sans SC', sans-serif; text-shadow: 0 0 10px rgba(0,243,255,0.4); 
        background: linear-gradient(90deg, rgba(0,243,255,0.1), transparent); 
        padding-top: 8px; padding-bottom: 8px; border-radius: 4px; 
    }
    ...
    /* 终端容器 */
    .terminal-container { 
        background: rgba(8, 15, 30, 0.85); border: 1px solid rgba(0,243,255,0.4); 
        padding: 25px; border-radius: 8px; font-family: 'Fira Code', monospace; 
        font-size: 14px; color: #e2e8f0; box-shadow: inset 0 0 20px rgba(0,243,255,0.1), 0 10px 30px rgba(0,0,0,0.8); 
        margin-bottom: 30px; 
    }
    .cursor-blink { 
        display: inline-block; width: 10px; height: 18px; background: #00f3ff; 
    ...
    div[data-testid="stTextInput"] > div > div > input { 
        background-color: rgba(4, 9, 20, 0.9) !important; color: #00f3ff !important; 
        font-family: 'Orbitron', monospace !important; border: 1px solid rgba(0,243,255,0.5) !important; 
        border-radius: 6px !important; text-align: center; font-size: 18px !important; 
        font-weight: bold !important; letter-spacing: 2px; box-shadow: inset 0 0 20px rgba(0,243,255,0.1) !important; 
        transition: all 0.3s ease; height: 50px; 
    }
    div[data-testid="stTextInput"] > div > div > input:focus { 
        border-color: #ffd700 !important; box-shadow: 0 0 25px rgba(255,215,0,0.4), inset 0 0 15px rgba(255,215,0,0.1) !important; 
    }

    /* 🚨 选项按钮：绝对不可放入 HTML 标签，已通过纯文本处理 */
    div.stButton > button { 
        background: linear-gradient(135deg, #0f172a 0%, #040914 100%) !important; 
        border: 1px solid rgba(0, 243, 255, 0.3) !important; border-left: 4px solid rgba(0, 243, 255, 0.6) !important; 
        border-radius: 6px !important; min-height: 60px !important; width: 100% !important; 
        padding: 10px 15px !important; text-align: left !important; box-shadow: 0 4px 10px rgba(0,0,0,0.4) !important; 
        transition: all 0.2s ease !important; margin-bottom: 5px !important;
    }
    ...
    div.stButton > button[data-testid="baseButton-primary"] p { 
        color: #010308 !important; font-weight: 900 !important; font-size: 18px !important; letter-spacing: 2px !important; 
    }

    /* 大屏卡片 */
    .result-card { 
        padding: 40px 30px; border-radius: 12px; background: rgba(8, 15, 30, 0.95) !important; 
        border: 1px solid rgba(255,215,0,0.3); border-top: 6px solid #ffd700; text-align: center; 
        box-shadow: 0 20px 50px rgba(0,0,0,0.8), inset 0 0 30px rgba(255,215,0,0.05); margin-bottom: 20px; 
        position:relative; overflow:hidden;
    }
    .mbti-code { 
        font-family: 'Orbitron', sans-serif !important; font-size: clamp(55px, 8vw, 80px); 
        font-weight: 900; color: #ffd700 !important; line-height: 1.1; letter-spacing: 6px; 
        text-shadow: 0 0 35px rgba(255,215,0,0.6); margin: 5px 0;
    }
    .tier-badge { 
        position: absolute; top: 20px; right: -45px; background: #ffd700; color: #000; 
        font-family: 'Orbitron', sans-serif; font-weight: 900; font-size: 13px; 
        padding: 5px 50px; transform: rotate(45deg); z-index: 10; letter-spacing: 2px; box-shadow: 0 0 20px rgba(255,215,0,0.8);
    }

    /* Tabs 全息导航坞 */
    [data-testid="stTabs"] button { 
        color: #64748b !important; font-family: 'Noto Sans SC', sans-serif !important; font-weight: 900 !important; 
        font-size: clamp(13px, 2vw, 15px) !important; padding-bottom: 12px !important; transition: all 0.3s ease; 
    }
    [data-testid="stTabs"] button[aria-selected="true"] { 
        color: #00f3ff !important; border-bottom-color: #00f3ff !important; border-bottom-width: 3px !important; 
        text-shadow: 0 0 15px rgba(0,243,255,0.6); background: rgba(0,243,255,0.05); 
    }
    ...
    div[data-testid="stCodeBlock"] pre, div[data-testid="stCodeBlock"] code { 
        font-family: 'Fira Code', monospace !important; font-size: 12px !important; color: #10b981 !important; line-height: 1.5 !important;
    }
    div[data-testid="stCodeBlock"] { border-left: 4px solid #10b981 !important; border-radius: 6px !important; margin-top: 10px; }

    /* 专属下载按钮 */
    div[data-testid="stDownloadButton"] > button { 
        background: rgba(5, 12, 25, 0.95) !important; border: 1px dashed rgba(16, 185, 129, 0.8) !important; 
        border-left: 6px solid #10b981 !important; margin-top: 20px; border-radius: 6px !important; 
        height: 55px; text-align: center !important; width: 100% !important;
    }
    div[data-testid="stDownloadButton"] > button p { 
        color: #10b981 !important; font-family: 'Orbitron', monospace !important; font-weight: bold !important; 
        letter-spacing: 2px !important; font-size: 14px !important;
    }
    div[data-testid="stDownloadButton"] > button:hover { 
        background: rgba(16, 185, 129, 0.15) !important; box-shadow: 0 0 30px rgba(16,185,129,0.5) !important; transform: scale(1.02) !important; 
    }

    /* 🌟 版权呼吸灯特效 */
    .copyright-niliu { 
        display: inline-block; padding: 12px 35px; border-radius: 50px; font-size: 13px; 
        font-family: "Noto Sans SC", sans-serif; letter-spacing: 2px; color: #00f3ff; font-weight: 900; 
        background: rgba(0,243,255,0.05); border: 1px solid rgba(0,243,255,0.3); 
        animation: neon-breathe 2.5s infinite alternate; transition: all 0.3s ease; box-shadow: 0 0 20px rgba(0,243,255,0.2); margin-bottom: 20px;
    }
    .copyright-niliu:hover { 
        transform: scale(1.05); box-shadow: 0 0 30px rgba(0,243,255,0.8), inset 0 0 15px rgba(0,243,255,0.5); border-color: #00f3ff; text-shadow: 0 0 10px #00f3ff; cursor: crosshair;
    }
    @keyframes neon-breathe { 
        0% { box-shadow: 0 0 10px rgba(0,243,255,0.1), inset 0 0 5px rgba(0,243,255,0.1); border-color: rgba(0,243,255,0.2); text-shadow: none; } 
        100% { box-shadow: 0 0 25px rgba(0,243,255,0.6), inset 0 0 15px rgba(0,243,255,0.2); border-color: rgba(0,243,255,0.7); text-shadow: 0 0 10px #00f3ff; } 
    }

    /* 🎆 结算科技烟花 (全息文字代码粒子) */
    .firework-center { 
        position: fixed; top: 50%; left: 50%; z-index: 99998; pointer-events: none; font-weight: 900; 
        font-family: 'Orbitron', monospace; color: #00f3ff; text-shadow: 0 0 20px #00f3ff, 0 0 30px #ffffff; 
        animation: supernova 1.8s cubic-bezier(0.1, 0.9, 0.2, 1) forwards;
    }
    @keyframes supernova { 
        0% { transform: translate(-50%, -50%) scale(0.1) rotate(0deg); opacity: 1; } 
        100% { transform: translate(calc(-50% + var(--tx)), calc(-50% + var(--ty))) scale(var(--s)) rotate(var(--rot)); opacity: 0; filter: blur(2px);} 
    }
</style>
""")
...
# ==============================================================================
# 🗃️ [ CORE 05 ] 18维满血防爆字典：16型大厂人格画像 (完全展开，确保零报错)
# ==============================================================================
mbti_details = {
    "INTJ": {
        ...
        "ultimate_evolution": "【绝对算力主宰】掌控数据产品的终极业务定价权", 
        ...
        "patch": "在构建宏大的交易规则体系时，请适当为前台业务预留“沙盒容错”空间。"
    },
    "INTP": {
        ...
        "ultimate_evolution": "【全知算法先知】构建百分百无损的跨网底层风控引擎", 
    ...
    "ISTJ": {
        ...
        "ultimate_evolution": "【绝对防御壁垒】全国数据市场要素流转网络的终极守门人", 
    ...
    "ESTJ": {
        ...
        "ultimate_evolution": "【全域秩序引擎】宏观数据业务推进的心脏中枢", 
    ...
    "INFP": {
        ...
        "ultimate_evolution": "【灵魂共振奇核】赋予极客冷数据极其昂贵的品牌溢价", 
    ...
    "ENTJ": {
        ...
        "ultimate_evolution": "【无界版图霸主】全国一体化数据市场的超级统帅", 
    ...
    "ENTP": {
        ...
        "ultimate_evolution": "【范式秩序破坏者】亲手定义下个十年的交易元规则", 
    ...
    "ENFJ": {
        ...
        "ultimate_evolution": "【共识引力波】垄断全国超头数据商的绝对心智", 
    ...
    "ENFP": {
        ...
        "ultimate_evolution": "【无界传播基站】把控国家全域要素市场的流量高地", 
    ...
    "ISFJ": {
        ...
        "ultimate_evolution": "【绝对永动节点】维持交易所生命线的最终坚盾", 
    ...
    "ESFJ": {
        ...
        "ultimate_evolution": "【政企超导桥梁】构筑不可替代的 G 端业务护城河", 
    ...
    "ISTP": {
        ...
        "ultimate_evolution": "【底层代码幽灵】掌控国家要素机房的绝对生命力", 
    ...
    "ISFP": {
        ...
        "ultimate_evolution": "【感官具象师】以一己之力拉升数据产品视觉溢价", 
    ...
    "ESTP": {
        ...
        "ultimate_evolution": "【极速套利猎手】全网数据交易套利空间的绝杀狙击者", 
    ...
    "ESFP": {
        ...
        "ultimate_evolution": "【极速情绪信标】左右资本市场情绪波动的首席发声端", 
...
if not st.session_state.started:
    render_html("<div style='margin-top:80px;'></div>")
    render_html("""
    <div style="max-width: 650px; margin: 0 auto;">
        ...
        <div class="terminal-container">
            ...
            <div class="term-line"><span style="color:#94a3b8;">[OK]</span> Securing root connection to SDE Ledger.</div>
            <div class="term-line"><span style="color:#94a3b8;">[OK]</span> Loading 40-Node Matrix Algorithm V1.0.</div>
    ...
    with st.form(key="login_form", border=False):
        render_html("<div style='color:#00f3ff; font-family:\"Orbitron\", sans-serif; font-size:12px; font-weight:bold; margin-bottom:8px; text-align:center;'>▼ MOUNT NODE IDENTIFIER (输入职场称呼 / 授权代号) ▼</div>")
    ...
    render_html("<div style='max-width: 650px; margin: 0 auto;'><h2 class='hero-title' data-text='[ HASHING NEURAL NETWORK ]' style='font-size:28px !important; margin-top:50px; text-align:center; display:block;'>[ HASHING NEURAL NETWORK ]</h2></div>")
    mint_box = st.empty()
    h_logs = ""
    for _ in range(12):
        ...
        mint_box.markdown(f"<div style='max-width: 650px; margin: 0 auto;'><div class='cli-box' style='height:250px; overflow:hidden; border-color:#ffd700;'>{h_logs}</div></div>", unsafe_allow_html=True)
        time.sleep(0.12)
    ...
    render_html("<div style='padding-top:10px;'></div>")
    progress_val = (st.session_state.current_q + 1) / len(questions)
    st.progress(progress_val)
    render_html(f"<div style='text-align:right; font-family:Orbitron, monospace; color:#00f3ff; font-size:12px; margin-top:5px;'>MINTING PROCESS: {int(progress_val*100)}%</div>")
    ...
    # 💠 [模块 I]：链上确权凭证 (SBT)
    render_html("<div class='module-title'>💠 模块 I：链上确权状态</div>")
    render_html(f"""
    ...
    # 🧬 [模块 II]：核心图谱大卡
    render_html("<div class='module-title'>🧬 模块 II：核心资产图谱</div>")
    ...
    with col_card_r:
        # 🤝 [模块 III]：跨域算力协同沙盘
        render_html("<div class='module-title' style='margin-top: 0;'>🤝 模块 III：跨域协同沙盘</div>")
        render_html("<div style='font-size:14px; color:#94a3b8; margin-bottom:15px; line-height:1.6;'>输入团队成员或领导的架构代码，系统将进行底层匹配：</div>")
        ...
        render_html("<h5 style='color:#10b981; margin-top:25px; margin-bottom:10px;'>💡 黄金搭档建议：</h5>")
        render_html(f"""
    ...
    # 🕸️ [模块 IV]：雷达矩阵与风控仪
    render_html("<div class='module-title'>🕸️ 模块 IV：算力雷达与风控仪</div>")
    col_mid_l, col_mid_r = st.columns([1, 1], gap="large")
    
    with col_mid_l:
        render_html("<div style='text-align:center; color:#00f3ff; font-weight:bold; margin-bottom:10px; font-family:Orbitron;'>/// RADAR MATRIX</div>")
        fig_radar = go.Figure()
    ...
    with col_mid_r:
        render_html("<div style='text-align:center; color:#f43f5e; font-weight:bold; margin-bottom:10px; font-family:Orbitron;'>/// RISK THRESHOLD GAUGE</div>")
        render_html(f"<div style='text-align:center; font-size:18px; font-weight:900; color:{r_color}; font-family:Noto Sans SC; margin-top: 15px; margin-bottom: 10px;'>【 {r_tag} 】</div>")
        ...
        fig_gauge.update_layout(paper_bgcolor='rgba(0,0,0,0)', font={'color': "#94a3b8"}, height=240, margin=dict(l=30, r=30, t=10, b=10))
        st.plotly_chart(fig_gauge, use_container_width=True, config={'displayModeBar': False}, theme=None)
        render_html(f"<div style='text-align:center; font-size:14px; color:#e2e8f0; line-height:1.6; padding: 0 15px; margin-bottom:20px;'>{r_desc}</div>")

    # 🗄️ [模块 V]：算力深潜控制台
    render_html("<div class='module-title'>🗄️ 模块 V：算力深潜控制台 (DEEP DIVE)</div>")
    ...
    with t_mkt:
        render_html("<div class='panel-title' style='color:#ffd700; border-color:#ffd700; margin-top:15px;'>/// 30-DAY MARKET ROI SIMULATION (ALPHA)</div>")
        render_html(f"<div style='font-size:14px; color:#e2e8f0; margin-bottom:10px; text-align:center;'>基于随机游走推演的交易风格：<br><span style='color:#ffd700; font-weight:bold; font-size:15px;'>【 {data['market_style']} 】</span></div>")
        fig_roi = go.Figure()
    ...
    with t_3d:
        render_html("<div class='panel-title' style='color:#00f3ff; border-color:#00f3ff; margin-top:15px;'>/// 3D COGNITIVE TOPOLOGY MAP</div>")
        render_html("<div style='font-size:13px; color:#94a3b8; margin-bottom:10px; text-align:center;'>降维映射至三维空间 (支持鼠标/触控 360° 拖拽)。背景点阵为抽样全网节点。</div>")
    ...
    with t_sol:
        render_html("<div class='panel-title' style='color:#10b981; border-color:#10b981; margin-top:15px;'>/// SOLIDITY SMART CONTRACT MINT LOG</div>")
        render_html("<div style='font-size:12px; color:#94a3b8; margin-bottom:10px; text-align:center;'>系统已自动为您生成专属的以太坊 ERC721 确权智能合约源码。</div>")
        ...
        render_html("<div class='panel-title' style='color:#f43f5e; border-color:#f43f5e; margin-top:20px; border-bottom:none;'>/// TOP SECRET DIRECTIVES ///</div>")
        ...
        render_html(f"<div style='margin-bottom: 10px;'>{tasks_html}</div>")

    # 📥 [模块 VI]：资产提取与分享终端
    render_html("<div class='module-title'>📥 模块 VI：资产全息提取终端</div>")
    t_img, t_txt, t_json = st.tabs(["📸 防白屏全息海报 (长按发圈)", "📝 纯文本通讯协议", "💾 极客 JSON 底包档案"])

    with t_img:
        render_html("<div style='font-size:13px; color:#10b981; margin-bottom:10px;'>系统已启用最高优先级【防显存溢出引擎】压制高清海报，请等待 2 秒...</div>")
...
<div style="text-align:center;font-size:11px;color:#94a3b8;margin-bottom:15px;">GLOBAL RARITY: <span style="color:{tier_color};font-family:Orbitron;font-weight:bold;font-size:13px;">{data['rarity']}</span></div>
...
<div style="font-size:12px; font-weight:bold; color:#fff; text-shadow:0 0 10px rgba(255,0,60,0.6);">{data['ultimate_evolution']}</div>
</div>
...
if (document.readyState === 'complete') {{
    ...
    with t_txt:
        render_html("<div style='font-size:13px; color:#94a3b8; margin-bottom:10px; margin-top:10px;'>👇 点击下方代码框右上角的 <b style='color:#00f3ff;'>Copy</b> 图标，复制纯文字名片供群聊使用：</div>")
...
🔗 [Token ID: #{token_id} | Hash: 0x{hash_code[:8]}]"""
    ...
    with t_json:
        render_html("<div style='font-size:13px; color:#94a3b8; margin-bottom:15px; margin-top:10px;'>💾 高管/极客视角：导出您的原生底层 JSON 结构树归档：</div>")
        export_data = {
            "version": VERSION,
