...
<style>
    ...
    /* 2. 丝滑转场动画 */
    [data-testid="stAppViewBlockContainer"] { animation: smoothFadeIn 0.6s cubic-bezier(0.22, 1, 0.36, 1); }
    @keyframes smoothFadeIn { 0% { opacity: 0; transform: translateY(15px); filter: blur(3px); } 100% { opacity: 1; transform: translateY(0); filter: blur(0); } }
    ...
    .hero-subtitle { text-align: center; color: #00f3ff !important; font-size: 14px; letter-spacing: 6px; opacity: 0.9; margin-bottom: 30px; font-family: 'Orbitron', sans-serif !important; font-weight: 700; }
    ...
    button[data-testid="baseButton-secondary"] {
        background-color: rgba(15, 23, 42, 0.7) !important; border: 1px solid rgba(0, 243, 255, 0.2) !important; 
        border-radius: 10px !important; min-height: 65px !important; width: 100% !important;
        padding: 12px 20px !important; text-align: left !important; backdrop-filter: blur(10px) !important;
        box-shadow: 0 4px 15px rgba(0,0,0,0.5) !important; transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1) !important;
    }
    button[data-testid="baseButton-secondary"] p { color: #ffffff !important; font-size: 15px !important; line-height: 1.6 !important; white-space: normal !important; font-weight: 400 !important; letter-spacing: 0.5px;}
    button[data-testid="baseButton-secondary"]:hover { background-color: rgba(0, 243, 255, 0.15) !important; border-color: #00f3ff !important; border-left: 4px solid #00f3ff !important; box-shadow: 0 0 20px rgba(0,243,255,0.3) !important; transform: translateX(6px) !important; }
    button[data-testid="baseButton-secondary"]:hover p { color: #00f3ff !important; text-shadow: 0 0 8px rgba(0,243,255,0.5); font-weight: 700 !important;}
    button[data-testid="baseButton-secondary"]:active { transform: scale(0.98) !important; }

    /* 5. 靶向接管操作按钮 */
    button[data-testid="baseButton-primary"] {
        background: linear-gradient(90deg, #00f3ff, #0088ff) !important; border: none !important; border-radius: 10px !important;
    ...
    button[data-testid="baseButton-primary"] p { color: #030712 !important; font-weight: 900 !important; font-size: 18px !important; letter-spacing: 2px !important; font-family: 'Noto Sans SC', sans-serif !important;}
    button[data-testid="baseButton-primary"]:hover { transform: translateY(-3px) scale(1.02) !important; box-shadow: 0 0 40px rgba(0,243,255,0.8) !important; filter: brightness(1.2); }
    ...
    @media print {
        body, .stApp, [data-testid="stAppViewContainer"] { background: #030712 !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
        button, .stProgress, [data-testid="stHeader"] { display: none !important; }
        .result-card { page-break-inside: avoid; border: 2px solid #334155 !important; }
    }
</style>
...
if not st.session_state.started:
    ...
    st.markdown("<div class='hero-subtitle'>▶ GENESIS_EDITION V21.0</div>", unsafe_allow_html=True)
    ...
    <div style='background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(0,243,255,0.4); padding: 30px; border-radius: 12px; font-family: monospace; font-size: 14px; color: #e2e8f0; box-shadow: inset 0 0 20px rgba(0,243,255,0.1), 0 10px 30px rgba(0,0,0,0.8); margin-bottom: 25px;'>
        ...
        <span style='color:#ffffff; font-size: 15px; font-family: "Noto Sans SC", sans-serif; line-height: 1.8;'>在数据要素化的高速演进中，坚实的风控合规是生态扩张的唯一基石。本终端将深度测算您的<b>底层能力雷达、决策模型与组织协作潜能</b>，生成不可篡改的个人数字凭证。</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("▶ 授予系统权限并启动 (AUTHORIZE & INIT)", type="primary", use_container_width=True):
        start_assessment(input_alias)
        st.rerun()
    
    st.markdown("<div style='text-align:center; color:#475569; font-size:12px; margin-top:50px; font-family:monospace;'>END-TO-END ENCRYPTED · AUDIT TRAIL ENABLED</div>", unsafe_allow_html=True)
    ...
    opts = [
        ("<span class='orbitron-font' style='color:#00f3ff; margin-right:8px;'>[ 00% ]</span> 拒绝指令：完全背离业务直觉", 1),
        ("<span class='orbitron-font' style='color:#00f3ff; margin-right:8px;'>[ 25% ]</span> 偏离轨道：较不符合执行习惯", 2),
        ("<span class='orbitron-font' style='color:#00f3ff; margin-right:8px;'>[ 50% ]</span> 待定挂起：视具体交易场景而定", 3),
        ("<span class='orbitron-font' style='color:#00f3ff; margin-right:8px;'>[ 75% ]</span> 拟合特征：比较符合决策链路", 4),
        ("<span class='orbitron-font' style='color:#00f3ff; margin-right:8px;'>[ 100% ]</span> 强制锁定：完全复刻底层逻辑", 5)
    ]
    ...
    # ✨ 新增：极客 CLI 命令行编译动画
    st.markdown("<h2 style='color:#00f3ff; font-family:\"Orbitron\", sans-serif; letter-spacing:4px; text-shadow:0 0 15px rgba(0,243,255,0.8); text-align:center;'>[ SYSTEM DECODING ]</h2>", unsafe_allow_html=True)
    terminal = st.empty()
    logs = [
        ...
        "GENERATING UNIQUE CRYPTOGRAPHIC SIGNATURE...",
    ...
    <div class="result-card">
        <div class="orbitron-font" style="font-size:14px; color:#94a3b8; letter-spacing:4px; margin-bottom:15px;">ASSET DECODED SUCCESSFULLY</div>
        <div style="color:#00f3ff; font-family:'Orbitron', monospace; font-size:14px; margin-bottom:5px; border-bottom:1px dashed #334155; padding-bottom:10px; display:inline-block;">AUTH_NODE: {st.session_state.user_alias.upper()}</div>
    ...
    st.markdown("<h4 style='color:#00f3ff !important; border-left:5px solid #00f3ff; padding-left:12px; font-weight:900;'>🎛️ 业务风控与决策仪表</h4>", unsafe_allow_html=True)
    ...
    fig_gauge = go.Figure(go.Indicator(mode="gauge+number", value=risk_score, number={'suffix': "%", 'font': {'family': 'Orbitron, sans-serif', 'color': r_color, 'size': 40}}, title={'text': f"<span style='color:{r_color}; font-size:16px; font-family:Noto Sans SC;'>{r_tag}</span>"}, gauge={'axis': {'range': [0, 100], 'tickwidth': 1, 'tickcolor': "#334155"}, 'bar': {'color': r_color}, 'bgcolor': "rgba(255,255,255,0.05)", 'steps': [{'range': [0, 35], 'color': "rgba(74, 222, 128, 0.15)"}, {'range': [35, 65], 'color': "rgba(255, 215, 0, 0.15)"}, {'range': [65, 100], 'color': "rgba(244, 63, 94, 0.15)"}]}))
    ...
    <div style='background: linear-gradient(145deg, rgba(16, 185, 129, 0.08), rgba(0,0,0,0)); border-left: 4px solid #10b981; padding: 25px; border-radius: 12px; font-size: 14.5px; line-height: 1.8; color: #e2e8f0 !important; border-top: 1px solid rgba(16, 185, 129, 0.3); border-bottom: 1px solid rgba(16, 185, 129, 0.3); box-shadow: 0 10px 30px rgba(0,0,0,0.5); margin: 15px 0 30px 0;'>
        <div style='color: #34d399 !important; font-weight: 900; font-size: 14px; margin-bottom: 10px; font-family: "Orbitron", sans-serif !important; letter-spacing: 1px;'>[ 黄金生态协作节点 ]</div>
        <div style='margin-bottom:15px; color:#ffffff !important; font-weight:900; font-size:16px;'>{data['partner']}</div>
        <div style='color: #34d399 !important; font-weight: 900; font-size: 14px; margin-bottom: 10px; font-family: "Orbitron", sans-serif !important; letter-spacing: 1px;'>[ 高阶算力提升指令 ]</div>
        <div>{data['advice']}</div>
    </div>
    """, unsafe_allow_html=True)
...
全息校验码：0x{hash_code}
（解码职场潜能，驱动要素未来）"""
    st.markdown(f"<div style='background:#050a15; padding:25px; border-radius:12px; font-family:\"Noto Sans SC\", monospace; font-size:14px; color:#ffffff !important; border:1px solid #00f3ff; box-shadow:0 0 25px rgba(0,243,255,0.15); white-space:pre-wrap; line-height:1.7;'>{share_card}</div>", unsafe_allow_html=True)
    
    st.caption("☝️ 提示：您可以长按上方文本框分享至微信，或直接使用浏览器【打印】功能生成专属高清数字档案。")
...
st.markdown("""
    <div style='text-align:center; margin-top:60px; margin-bottom:30px; font-family:\"Orbitron\", monospace;'>
